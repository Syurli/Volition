...
import { envelope } from '@willform/protocol';
import { runTacticalWizardFixture } from '@willform/example-tactical-wizard';
import {
...
describe('Workbench live endpoint safety', () => {
  it('rejects ws mixed content from HTTPS Pages', () => {
    const result = validateLiveEndpoint('ws://localhost:7000/willform', 'https:');
    expect(result.valid).toBe(false);
    expect(result.message).toContain('mixed content');
  });

  it('allows wss in production and ws in local HTTP development', () => {
    expect(validateLiveEndpoint('wss://localhost:7443/willform', 'https:').valid).toBe(true);
    expect(validateLiveEndpoint('ws://localhost:7000/willform', 'http:').valid).toBe(true);
  });
