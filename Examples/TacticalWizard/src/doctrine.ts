...
export function decideSquadDoctrine(current: SquadTactic, facts: SquadDoctrineFacts): SquadDoctrineDecision {
  ...
  switch (current) {
    case 'bounding':
      if (facts.boundingPhase >= 1 && planCompletion >= 1 && facts.stationaryTargetTicks >= 8 && facts.tacticTicks >= 6) return { tactic: 'flank', reason: 'The bound completed and the target is still holding; pin it while a committed flanking element opens a rear-quarter angle.', rotateRoles: true };
    ...
    case 'crossfire':
      if (planCompletion >= 1 && stableContactTicks >= 3 && facts.tacticTicks >= 6) return { tactic: 'assault', reason: 'Both crossfire elements reached separated sectors and contact is stable; exploit the established geometry with a coordinated assault.', rotateRoles: true };
    ...
    case 'regroup':
      if (planCompletion >= 1 && facts.tacticTicks >= 8) return { tactic: 'bounding', reason: `At least two members recovered their committed spacing for maneuver cycle ${maneuverCycle + 1}; resume pressure with a new role order and opposite flank preference.`, rotateRoles: true };
