...
export type LocomotionMode = 'free' | 'forward' | 'lateral' | 'backpedal' | 'covered_dash';
export type BuddyRole = 'none' | 'lead' | 'cover' | 'overwatch';
export type TacticalOpportunityPurpose = 'none' | 'base_of_fire' | 'covered_move' | 'flank' | 'crossfire' | 'block_exit' | 'search' | 'ambush' | 'close_pressure';
export type GrenadeKind = 'flash' | 'frag' | 'smoke';
export type SpecialAction = 'none' | 'throw_flash' | 'throw_frag' | 'throw_smoke' | 'melee' | 'surprise';

export interface GrenadeVisual {
  readonly id: number;
...
export interface TacticalWizardAgentView extends BaseAgentView {
  ...
  readonly fireBlockedTicks: number;
  readonly searchLookTarget: GridPoint | null;
  readonly searchProgress: number;
  readonly locomotionMode: LocomotionMode;
  readonly backTurnPermitted: boolean;
  readonly buddyRole: BuddyRole;
  readonly buddyReady: boolean;
  readonly opportunityPurpose: TacticalOpportunityPurpose;
  readonly grenadeCount: number;
  readonly grenadeCooldownTicks: number;
  readonly specialAction: SpecialAction;
  readonly specialActionPulse: number;
  readonly meleePulse: number;
}
...
export interface TacticalWizardSimulationState extends Omit<BaseSimulationState, 'agents'> {
  ...
  readonly coordination: {
    readonly searchLeadId: string | null;
    readonly searchCoverId: string | null;
    readonly searchOverwatchId: string | null;
    readonly lastSquadGrenadeTick: number;
    readonly planRevision: number;
    readonly lastPlanReplanTick: number;
  };
}
...
const FIRE_BLOCKED_REPLAN_TICKS = 3;
const PLAN_REPLAN_COOLDOWN_TICKS = 4;
...
const PEEK_FRAMES = 9;
const COVERED_DASH_FRAMES = 18;
const COVERED_DASH_MIN_RANGE = 6.5;
const SQUAD_GRENADE_COOLDOWN_TICKS = 18;
const MEMBER_GRENADE_COOLDOWN_TICKS = 28;
const GRENADE_MIN_RANGE = 4;
const GRENADE_MAX_RANGE = 10;
const GRENADE_FRIEND_CLEARANCE = 2.25;
const GRENADE_VISUAL_FRAMES = 72;
const GRENADE_FLIGHT_FRAMES = 18;
const MELEE_RANGE = 1.35;
const MELEE_COOLDOWN_TICKS = 10;

interface MutableGrenade extends GrenadeVisual {
...
interface MutableMember {
  ...
  backTurnCommitUntilFrame: number;
  backTurnPermitted: boolean;
  buddyRole: BuddyRole;
  buddyReady: boolean;
  opportunityPurpose: TacticalOpportunityPurpose;
  grenadeCount: number;
...
export class TacticalWizardSimulation {
  ...
  private searchLeadId: string | null = null;
  private searchCoverId: string | null = null;
  private searchOverwatchId: string | null = null;
  private lastSquadGrenadeTick = -999;
  private grenadeSequence = 0;
  private activeGrenades: MutableGrenade[] = [];
  ...
  reset(): TacticalWizardSimulationState {
    ...
    this.searchCoverId = null;
    this.searchOverwatchId = null;
    this.lastSquadGrenadeTick = -999;
  ...
  getState(): TacticalWizardSimulationState {
    ...
    const views: TacticalWizardAgentView[] = this.members.map((member) => ({
      ...
      fireBlockedTicks: member.fireBlockedTicks,
      searchLookTarget: member.searchLookTarget,
      searchProgress: this.memberSearchProgress(member),
      locomotionMode: member.locomotionMode,
      backTurnPermitted: member.backTurnPermitted,
      buddyRole: member.buddyRole,
      buddyReady: member.buddyReady,
      opportunityPurpose: member.opportunityPurpose,
      grenadeCount: member.grenadeCount,
      grenadeCooldownTicks: Math.max(0, member.grenadeCooldownUntilTick - this.logicalTick),
      specialAction: member.specialAction,
      specialActionPulse: member.specialActionPulse,
      meleePulse: member.meleePulse,
    }));
    ...
    return {
      ...
      grenadeEvents: this.activeGrenades.map((grenade) => ({ ...grenade })),
      coordination: {
        searchLeadId: this.searchLeadId,
        searchCoverId: this.searchCoverId,
        searchOverwatchId: this.searchOverwatchId,
        lastSquadGrenadeTick: this.lastSquadGrenadeTick,
        planRevision: this.planRevision,
        lastPlanReplanTick: this.lastPlanReplanTick,
      },
    };
  }
  ...
  private advanceDecisionTick(): void {
    ...
    for (const member of this.members) {
      ...
      const values = {
        ...
        locomotionMode: member.locomotionMode,
        buddyRole: member.buddyRole,
        tacticalPurpose: member.opportunityPurpose,
        grenadeCount: member.grenadeCount,
        fireBlockedTicks: member.fireBlockedTicks,
  ...
  private advanceMotionFrame(): void {
    this.activeGrenades = this.activeGrenades.flatMap((grenade) => grenade.remainingFrames <= 1 ? [] : [{ ...grenade, remainingFrames: grenade.remainingFrames - 1 }]);
    for (const member of this.members) {
      ...
      member.specialActionPulse = Math.max(0, member.specialActionPulse - 1);
      member.meleePulse = Math.max(0, member.meleePulse - 1);
      if (member.specialActionPulse === 0) member.specialAction = 'none';
      if (member.coverState === 'peeking' && this.motionFrame >= member.peekUntilFrame) member.coverState = member.coverSlot === null ? 'none' : 'covered';
      if (member.backTurnCommitUntilFrame < this.motionFrame) member.backTurnPermitted = false;
    }
  ...
  private movementTarget(member: MutableMember): GridPoint | null {
    ...
    if (this.tactic === 'sweep') {
      if (member.task === 'overwatch') {
        if (member.searchWaypoints.length === 0) return null;
        const target = member.searchWaypoints[0]!;
        return distance(member.position, target) <= ARRIVAL_RADIUS ? null : target;
      }
      if (member.id === this.searchCoverId) return null;
      if (member.id !== this.searchLeadId || member.searchComplete || member.searchWaypoints.length === 0) return null;
  ...
  private updateSearchScan(member: MutableMember): void {
    ...
    if (member.id === this.searchCoverId) {
      const lead = this.searchLeadId === null ? undefined : this.members.find((candidate) => candidate.id === this.searchLeadId);
      const leadTarget = lead?.searchWaypoints[Math.min(lead.searchIndex, Math.max(0, lead.searchWaypoints.length - 1))] ?? lead?.position ?? lkp;
      member.searchLookTarget = leadTarget;
      member.buddyReady = hasWorldLineOfSight(this.fireOriginFor(member), leadTarget);
      member.coverState = member.coverSlot === null ? 'none' : 'covered';
      member.searchPulse = Math.max(member.searchPulse, 6);
      this.faceToward(member, leadTarget);
      return;
    }

    if (member.searchWaypoints.length === 0 || member.searchComplete) return;
    ...
    if (distance(member.position, waypoint) > ARRIVAL_RADIUS) {
      ...
      member.searchLookTarget = waypoint;
      member.buddyReady = member.task === 'overwatch';
      return;
    }
    ...
    if (member.searchIndex >= member.searchWaypoints.length) {
      ...
      member.searchIndex = Math.max(0, member.searchWaypoints.length - 1);
      this.log('agent', member.id, member.label, 'search', `${member.label} completed its assigned search sector.`, { task: member.task, progress: 1, buddyRole: member.buddyRole });
    } else {
      const next = member.searchWaypoints[member.searchIndex]!;
      member.tacticalTarget = next;
      this.log('agent', member.id, member.label, 'search', `${member.label} cleared a scan point; buddy handoff before the next advance.`, { task: member.task, target: { ...next }, buddyRole: member.buddyRole });
    }
    this.handoffSearchBuddy(member);
  }

  private handoffSearchBuddy(completedLead: MutableMember): void {
    if (completedLead.id !== this.searchLeadId || this.searchCoverId === null) return;
    const nextLead = this.members.find((member) => member.id === this.searchCoverId);
    if (nextLead === undefined || nextLead.searchComplete) return;
    const oldLeadId = this.searchLeadId;
    this.searchLeadId = nextLead.id;
    this.searchCoverId = oldLeadId;
    for (const member of this.members) {
      if (member.id === this.searchLeadId) {
        member.buddyRole = 'lead';
        member.buddyReady = true;
        member.opportunityPurpose = 'search';
      } else if (member.id === this.searchCoverId) {
        member.buddyRole = 'cover';
        member.buddyReady = true;
        member.opportunityPurpose = 'base_of_fire';
      }
    }
    this.pushEvent(`T${this.logicalTick}: search buddy handoff; ${nextLead.label} advances while ${completedLead.label} covers.`);
    this.log('squad', 'twr:rifle-squad-01', 'Rifle Squad 01', 'roles', 'Search buddy roles handed off after a cleared scan point.', { lead: this.searchLeadId, cover: this.searchCoverId, overwatch: this.searchOverwatchId });
  }
  ...
  private executeDecisionEffects(): void {
    ...
    if (this.tactic === 'sweep') {
      for (const member of this.members) {
        if (this.tryGrenade(member)) break;
      }
      for (const member of this.members) {
        if (this.logicalTick % 4 === 0) this.log('agent', member.id, member.label, 'search', `${member.label} is executing coordinated search responsibility.`, { task: member.task, buddyRole: member.buddyRole, buddyReady: member.buddyReady, progress: Number(this.memberSearchProgress(member).toFixed(2)) });
      }
      return;
    }
    ...
    this.meleeClaimId = null;
    for (const member of this.members) {
      if (this.tryMelee(member)) continue;
      if (this.tryGrenade(member)) continue;
  ...
  private tryMelee(member: MutableMember): boolean {
    if (!member.targetVisible || this.logicalTick < member.meleeCooldownUntilTick || distance(member.position, this.player) > MELEE_RANGE) return false;
    if (this.meleeClaimId !== null && this.meleeClaimId !== member.id) return false;
    this.meleeClaimId = member.id;
    member.meleePulse = 8;
    member.specialAction = 'melee';
    member.specialActionPulse = 8;
    member.meleeCooldownUntilTick = this.logicalTick + MELEE_COOLDOWN_TICKS;
    member.opportunityPurpose = 'close_pressure';
    this.faceToward(member, this.player);
    this.pushEvent(`T${this.logicalTick}: ${member.label} claimed the close-pressure opportunity.`);
    this.log('agent', member.id, member.label, 'fire', `${member.label} used a close-pressure / melee opportunity.`, { task: member.task, tactic: this.tactic, distance: Number(distance(member.position, this.player).toFixed(2)) });
    return true;
  }

  private tryGrenade(member: MutableMember): boolean {
    if (member.grenadeCount <= 0 || this.logicalTick < member.grenadeCooldownUntilTick || this.logicalTick - this.lastSquadGrenadeTick < SQUAD_GRENADE_COOLDOWN_TICKS || this.sharedLastKnownPosition === null) return false;
    const target = this.sharedLastKnownPosition;
    const range = distance(member.position, target);
    if (range < GRENADE_MIN_RANGE || range > GRENADE_MAX_RANGE) return false;
    if (this.members.some((friendly) => friendly.id !== member.id && distance(friendly.position, target) <= GRENADE_FRIEND_CLEARANCE)) return false;

    let kind: GrenadeKind | null = null;
    let probability = 0;
    if (this.tactic === 'sweep' && this.lostContactTicks >= 2 && (member.buddyRole === 'lead' || member.buddyRole === 'overwatch')) {
      kind = 'flash';
      probability = 0.42;
    } else if (this.stationaryTargetTicks >= 8 && member.targetVisible) {
      kind = 'frag';
      probability = 0.32;
    } else if ((member.task === 'bound_to_cover' || member.task === 'flank_to_cover') && !this.hasSafeSupportingLane(member)) {
      kind = 'smoke';
      probability = 0.26;
    }
    if (kind === null || deterministicUnit(member.id, this.logicalTick, this.maneuverCycle) >= probability) return false;

    member.grenadeCount -= 1;
    member.grenadeCooldownUntilTick = this.logicalTick + MEMBER_GRENADE_COOLDOWN_TICKS;
    member.specialAction = kind === 'flash' ? 'throw_flash' : kind === 'frag' ? 'throw_frag' : 'throw_smoke';
    member.specialActionPulse = 18;
    this.lastSquadGrenadeTick = this.logicalTick;
    const radius = kind === 'smoke' ? 3.1 : kind === 'flash' ? 2.8 : 2.35;
    this.activeGrenades.push({
      id: this.grenadeSequence++, ownerId: member.id, kind,
      from: { ...member.position }, to: { ...target }, radius,
      remainingFrames: GRENADE_VISUAL_FRAMES, totalFrames: GRENADE_VISUAL_FRAMES, flightFrames: GRENADE_FLIGHT_FRAMES,
    });
    this.pushEvent(`T${this.logicalTick}: ${member.label} used ${kind} grenade as a space-control action.`);
    this.log('agent', member.id, member.label, 'fire', `${member.label} committed an abstract ${kind} grenade opportunity.`, { target: { ...target }, range: Number(range.toFixed(2)), task: member.task, tactic: this.tactic, grenadeCount: member.grenadeCount });
    return true;
  }

  private tryFire(member: MutableMember, target: GridPoint, reason: string): void {
    const origin = this.fireOriginFor(member);
    if (!hasWorldLineOfSight(origin, target)) {
      member.fireBlockedByFriend = null;
      member.fireBlockedTicks = 0;
      return;
    }
    const blockedBy = findFriendlyInFireLane(member.id, origin, target, this.members, FIRE_LANE_CLEARANCE);
    if (blockedBy !== null) {
      member.fireBlockedTicks += 1;
      if (member.fireBlockedByFriend !== blockedBy || member.fireBlockedTicks === FIRE_BLOCKED_REPLAN_TICKS) {
        member.fireBlockedByFriend = blockedBy;
        const blocker = this.members.find((entry) => entry.id === blockedBy)?.label ?? blockedBy;
        this.log('agent', member.id, member.label, 'fire', `${member.label} withheld fire because ${blocker} occupies the fire lane.`, { blocked: true, blockedBy, blockedTicks: member.fireBlockedTicks, task: member.task, tactic: this.tactic });
      }
      return;
    }
    member.fireBlockedByFriend = null;
  ...
  private hasSafeSupportingLane(member: MutableMember): boolean {
    return this.currentFireLanes().some((lane) => lane.ownerId !== member.id && findFriendlyInFireLane(lane.ownerId, lane.from, lane.to, this.members, FIRE_LANE_CLEARANCE) === null);
  }
  ...
  private ensureTacticalPlan(): void {
    ...
    if (persistentFireBlock) {
      ...
      this.log('squad', 'twr:rifle-squad-01', 'Rifle Squad 01', 'plan', 'Persistent friendly fire-lane conflict invalidated the current tactical plan.', { blockedMembers: blocked, planRevision: this.planRevision });
    }
    this.refreshTacticalPlan();
  }
  ...
  private refreshBoundingPlan(): void {
    ...
    for (const id of [this.suppressorId, this.moverId, this.observerId].filter((value): value is string => value !== null)) {
      ...
      member.opportunityPurpose = member.id === this.suppressorId ? 'base_of_fire' : member.id === this.moverId ? 'covered_move' : 'block_exit';
    }
  }
  ...
  private refreshFlankPlan(): void {
    ...
    for (const member of this.members) {
      if (member.id === this.moverId) {
        ...
        member.opportunityPurpose = 'flank';
      } else {
        ...
        member.opportunityPurpose = member.id === this.suppressorId ? 'base_of_fire' : 'block_exit';
      }
    }
  }
  ...
  private refreshCrossfirePlan(): void {
    ...
    shooters.forEach((id, index) => {
      ...
      member.opportunityPurpose = 'crossfire';
    });
    ...
    if (support !== undefined) {
      ...
      support.opportunityPurpose = 'base_of_fire';
    }
  }
  ...
  private refreshAssaultPlan(): void {
    ...
    assaulters.forEach((id, index) => {
      ...
      reserveCellArea(reserved, member.tacticalTarget, 1);
      member.task = 'assault';
      member.opportunityPurpose = 'close_pressure';
    });
    const support = this.suppressorId === null ? undefined : this.members.find((entry) => entry.id === this.suppressorId);
    if (support !== undefined) {
      const supportReserved = new Set(this.members.filter((member) => member.tacticalTarget !== null).map((member) => gridKey(toNavCell(member.tacticalTarget!))));
      ...
      support.coverSlot = slot;
      support.tacticalTarget = slot !== null && !supportReserved.has(gridKey(slot.position)) ? slot.position : { ...support.position };
  ...
  private refreshSweepPlan(): void {
    ...
    this.searchCoverId = searchers[1]?.id ?? null;
    this.searchOverwatchId = overwatch?.id ?? null;

    for (const member of ordered) {
      const patternIndex = member.id === this.searchOverwatchId ? 2 : member.id === this.searchLeadId ? 0 : 1;
      const pattern = makeSearchPattern(tacticalWizardNavigationGrid, toNavCell(lkp), patternIndex, side);
      member.searchWaypoints = member.id === this.searchOverwatchId ? pattern.waypoints.slice(0, 1) : pattern.waypoints;
      ...
      member.task = member.id === this.searchOverwatchId ? 'overwatch' : 'search_sector';
      member.buddyRole = member.id === this.searchLeadId ? 'lead' : member.id === this.searchCoverId ? 'cover' : 'overwatch';
      member.buddyReady = member.buddyRole !== 'lead';
      member.opportunityPurpose = member.id === this.searchOverwatchId ? 'block_exit' : member.id === this.searchCoverId ? 'base_of_fire' : 'search';
      member.coverState = 'none';
    }
    this.pushEvent(`T${this.logicalTick}: coordinated search contract assigned — lead, cover, overwatch.`);
  }
  ...
  private refreshRegroupPlan(): void {
    ...
    this.members.forEach((member, index) => {
      ...
      if (slot !== null) {
        ...
        member.tacticalTarget = point ?? { ...member.position };
        reserveCellArea(reservedCells, member.tacticalTarget, 1);
      }
      member.task = 'regroup';
      member.opportunityPurpose = 'block_exit';
    });
  }
  ...
  private buildStimuli(member: MutableMember, visible: boolean): readonly Stimulus[] {
    ...
    if (visible) {
      ...
      if (!member.wasVisible) {
        if (this.tactic === 'sweep' && member.opportunityPurpose === 'block_exit') {
          member.specialAction = 'surprise';
          member.specialActionPulse = 10;
          member.opportunityPurpose = 'ambush';
          this.pushEvent(`T${this.logicalTick}: ${member.label} reacquired contact from the blocking position.`);
        } else {
          this.pushEvent(`T${this.logicalTick}: ${member.label} visual contact confirmed.`);
        }
        this.log('agent', member.id, member.label, 'perception', `${member.label} acquired visual contact.`, { position: { ...this.player }, opportunityPurpose: member.opportunityPurpose });
      }
  ...
  private moveMemberToward(member: MutableMember, target: GridPoint, occupiedCells: ReadonlySet<string>, lanes: readonly FireLane[], maxDistance: number): void {
    ...
    const plannedDirection = normalizeDirection({ x: waypoint.x - member.position.x, y: waypoint.y - member.position.y });
    const locomotionMode = this.resolveLocomotionMode(member, plannedDirection);
    const speedScale = locomotionMode === 'backpedal' ? 0.58 : locomotionMode === 'lateral' ? 0.78 : 1;
    const candidate = moveToward(member.position, waypoint, maxDistance * speedScale);
    ...
    if (Math.abs(dx) > POSITION_EPSILON || Math.abs(dy) > POSITION_EPSILON) {
      const movementDirection = normalizeDirection({ x: dx, y: dy });
      member.locomotionMode = locomotionMode;
      this.applyLocomotionFacing(member, locomotionMode, movementDirection);
      member.stalledTicks = 0;
      if (this.motionFrame % MOTION_LOG_EVERY_FRAMES === 0) {
        this.log('agent', member.id, member.label, 'move', `${member.label} moved for tactical task ${member.task}.`, { from: { ...previous }, to: { ...member.position }, tacticalTarget: { ...target }, role: member.role, task: member.task, tactic: this.tactic, motionFrame: this.motionFrame, locomotionMode: member.locomotionMode, backTurnPermitted: member.backTurnPermitted });
      }
  ...
  private resolveLocomotionMode(member: MutableMember, movementDirection: GridPoint): LocomotionMode {
    if (this.alertState !== 'active' || this.sharedLastKnownPosition === null) {
      member.backTurnPermitted = false;
      return 'free';
    }
    const threatDirection = normalizeDirection({ x: this.sharedLastKnownPosition.x - member.position.x, y: this.sharedLastKnownPosition.y - member.position.y });
    const angle = vectorAngleDegrees(movementDirection, threatDirection);
    if (angle <= 58) {
      member.backTurnPermitted = false;
      return 'forward';
    }
    ...
    if (this.motionFrame <= member.backTurnCommitUntilFrame) {
      member.backTurnPermitted = true;
      return 'covered_dash';
    }
    const eligibleTask = member.task === 'bound_to_cover' || member.task === 'flank_to_cover' || member.task === 'assault';
    const committedDestination = member.coverSlot !== null || member.opportunityPurpose === 'flank' || member.opportunityPurpose === 'close_pressure';
    const supported = this.hasSafeSupportingLane(member);
    const enoughRange = distance(member.position, this.sharedLastKnownPosition) >= COVERED_DASH_MIN_RANGE;
    if (eligibleTask && committedDestination && supported && enoughRange) {
      const probability = this.safeFireLaneCount() >= 2 ? 0.28 : 0.14;
      if (deterministicUnit(member.id, this.logicalTick, this.motionFrame) < probability) {
        member.backTurnCommitUntilFrame = this.motionFrame + COVERED_DASH_FRAMES;
        member.backTurnPermitted = true;
        this.log('agent', member.id, member.label, 'move', `${member.label} committed a short supported covered dash.`, { task: member.task, supportLanes: this.safeFireLaneCount(), probability, untilFrame: member.backTurnCommitUntilFrame });
        return 'covered_dash';
      }
    }
    member.backTurnPermitted = false;
  ...
  private applyLocomotionFacing(member: MutableMember, mode: LocomotionMode, movementDirection: GridPoint): void {
    if (mode === 'free' || mode === 'forward' || mode === 'covered_dash' || this.sharedLastKnownPosition === null) {
      member.facing = movementDirection;
      return;
    }
  ...
  private resetMember(member: MutableMember): void {
    ...
    member.searchComplete = false;
    member.stalledTicks = 0;
    member.locomotionMode = 'free';
    member.backTurnCommitUntilFrame = 0;
    member.backTurnPermitted = false;
    member.buddyRole = 'none';
    member.buddyReady = false;
    member.opportunityPurpose = 'none';
...
function createMembers(): MutableMember[] {
  return MEMBER_DEFINITIONS.map((definition) => ({
    ...
    stalledTicks: 0,
    locomotionMode: 'free',
    backTurnCommitUntilFrame: 0,
    backTurnPermitted: false,
    buddyRole: 'none',
    buddyReady: false,
    opportunityPurpose: 'none',
    grenadeCount: 1,
...
function reserveCellArea(set: Set<string>, point: GridPoint | null, radius: number): void {
  if (point === null) return;
  const center = toNavCell(point);
  for (let y = center.y - radius; y <= center.y + radius; y += 1) for (let x = center.x - radius; x <= center.x + radius; x += 1) {
    if (x >= 0 && y >= 0 && x < tacticalWizardTestMap.width && y < tacticalWizardTestMap.height) set.add(gridKey({ x, y }));
  }
}
function isSustainedFireTask(task: TacticalTask): boolean { return task === 'suppress' || task === 'hold_cover' || task === 'crossfire'; }
...
function vectorAngleDegrees(left: GridPoint, right: GridPoint): number { const dot = Math.max(-1, Math.min(1, left.x * right.x + left.y * right.y)); return Math.acos(dot) * 180 / Math.PI; }
function deterministicUnit(id: string, a: number, b: number): number { let hash = 2166136261; const source = `${id}:${a}:${b}`; for (let index = 0; index < source.length; index += 1) { hash ^= source.charCodeAt(index); hash = Math.imul(hash, 16777619); } return (hash >>> 0) / 4294967296; }
