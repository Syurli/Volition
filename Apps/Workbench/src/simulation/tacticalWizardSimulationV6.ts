...
export class TacticalWizardSimulation {
  ...
  constructor() {
    this.log('system', 'simulation', 'Volition Simulation', 'session', 'V6 mutual-support tactical simulation started.', {
  ...
  reset(): TacticalWizardSimulationState {
    ...
    this.eventLog.splice(0, this.eventLog.length, 'Simulation reset. V6 mutual-support coordinator / 30 Hz motion initialized.');
    this.runLog = [];
    this.runLogSequence = 0;
    this.log('system', 'simulation', 'Volition Simulation', 'session', 'Simulation reset; V6 tactical run started.', {
      map: tacticalWizardTestMap.id,
      motionHz: MOTION_HZ,
      decisionHz: 1 / DECISION_INTERVAL_SECONDS,
      agentMoveSpeed: AGENT_MOVE_SPEED,
  ...
  private tryGrenade(member: MutableMember): boolean {
    ...
    if (this.tactic === 'sweep' && this.lostContactTicks >= 2 && (member.buddyRole === 'lead' || member.buddyRole === 'overwatch')) {
      kind = 'flash';
      probability = 0.34;
    } else if (this.stationaryTargetTicks >= 12 && member.targetVisible) {
      kind = 'frag';
      probability = 0.24;
    ...
    this.activeGrenades.push({ id: this.grenadeSequence++, ownerId: member.id, kind, from: { ...member.position }, to: { ...target }, radius, remainingFrames: 30 });
  ...
  private activateSquad(): void {
    ...
    this.log('squad', 'twr:rifle-squad-01', 'Rifle Squad 01', 'alert', 'Shared combat alert activated with V6 mutual-support task allocation.', { source: source.id, tactic: this.tactic });
