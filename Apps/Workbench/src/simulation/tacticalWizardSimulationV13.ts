...
interface HostAccess {
  ...
  currentFireLanes: () => readonly { readonly ownerId: string }[];
...
const COUNTERFIRE_RADIUS = 5;
const COUNTERFIRE_ARRIVAL = 1.0;
const COUNTERFIRE_INTERVAL_TICKS = 2;

/**
 * V13 is a reliability pass over the V9-V12 host stack.
 *
 * It fixes four concrete play-test failures without moving navigation, cover or
 * weapon simulation into Portable Core:
 *
 * 1. Downed actors are no longer valid perception / squad-role participants.
 * 2. Rescue security has one authoritative readiness gate instead of V10's
 *    timer and V11's physical-position gate repeatedly overwriting each other.
 * 3. Rescue security positions stay close enough to protect the casualty, must
 *    expose a real lane toward confirmed/shared/coarse threat memory, and abort
 *    cleanly after bounded replans instead of looping forever.
 * 4. Unseen incoming fire can establish a separate base-of-fire element toward
 *    the deliberately coarse inferred sector while the exposed member moves to
 *    safety. The exact hidden shooter position is never consumed.
 *
 * The hooks below deliberately adapt the existing reference Host rather than
 * promoting these sandbox semantics into Volition Core. They should become
 * explicit Host contracts when the reference slice is generalized.
 */
export class TacticalWizardSimulation extends TacticalWizardSimulationV12 {
  ...
  constructor() {
    ...
    const host = this.v13Host();
    host.pushEvent('V13: operational-roster filtering, bounded rescue security and unseen-fire counterfire enabled.');
    host.log('system', 'simulation', 'Volition Simulation', 'session', 'V13 rescue reliability / counterfire layer enabled.', {
      ...
      counterfireIntervalTicks: COUNTERFIRE_INTERVAL_TICKS,
    });
  }
  ...
  override reset(): TacticalWizardSimulationState {
    ...
    this.v13Host().pushEvent('V13: rescue reliability / operational roster / counterfire state reset.');
  ...
  private installOperationalHooks(): void {
    ...
    host.buildStimuli = (member: HostMember, visible: boolean): readonly unknown[] => {
      if (!this.isAlive(member.id)) return [];
    ...
    const originalFireLanes = host.currentFireLanes.bind(this);
    host.currentFireLanes = () => originalFireLanes().filter((lane) => this.isAlive(lane.ownerId));
  }

  private installRescueHooks(): void {
    const v10 = this.v13V10();
    const v11 = this.v13V11();
    const v12 = this.v13V12();
    ...
    v10.tryCreateRescuePlan = (state: MinimalState): void => {
      ...
      if (alive.length < 2) {
        ...
        if (this.rescueUnavailableKey !== key) {
          ...
          this.v13Host().log('squad', 'twr:rifle-squad-01', 'Rifle Squad 01', 'plan', 'Casualty recovery deferred: no separate living security element is available.', {
            downedId: downed.id,
    ...
    v10.advanceRecovery = (deltaSeconds: number): void => {
      ...
      if (plan.phase === 'establish_cover') {
        ...
        this.v13Host().pushEvent(`T${state.logicalTick}: rescue security lane is physically ready; casualty approach begins.`);
        this.v13Host().log('squad', 'twr:rifle-squad-01', 'Rifle Squad 01', 'plan', 'Rescue advanced only after the security element reached a usable threat-facing lane.', {
    ...
    // V11 used to reset V10's rescue phase every frame whenever its own cover
    // point was not ready. V13 makes V12/V13's fire-support position the single
    // readiness source, so this hook only validates the existence of the third
    // living security element and never rewinds the phase independently.
    ...
    // Replace V12's point selection with a casualty-local fire-support search.
    // The fire target is memory-safe: visible player, coarse inferred sector,
    // squad LKP, or an Agent's own recorded LKP. It never reads hidden live truth.
    v12.prepareRescueSupport = (state: TacticalWizardSimulationStateV12): void => {
      this.prepareRescueSupportV13(state);
    };
  }
  ...
  private installPriorityMovementHook(): void {
    ...
    host.movementTarget = (member: HostMember): GridPoint | null => {
      ...
      if (agent === undefined || !agent.alive || isHardReaction(agent.reactionState)) return base;

      const rescue = this.v13V10().rescuePlan;
      const rescuePosition = this.v13V12().rescueSupportPosition;
      if (rescue !== null && rescue.covererId === member.id && rescue.phase === 'establish_cover' && rescuePosition !== null) {
        return { ...rescuePosition };
      }

      if (state.threatResponse.active
        && state.threatResponse.phase === 'break_contact'
        && this.counterfireAgentId === member.id
        && this.counterfirePosition !== null) {
  ...
  private prepareRescueSupportV13(state: TacticalWizardSimulationStateV12): void {
    const plan = this.v13V10().rescuePlan;
    const support = this.v13V12();
    ...
    if (fireTarget === null) {
      support.rescueSupportTarget = null;
      support.rescueSupportPosition = null;
      this.v13V11().rescueCoverTarget = null;
      return;
    }
    ...
    if (support.rescueSupportPlanKey !== planKey || support.rescueSupportPosition === null || !currentLaneValid) {
      ...
      const selected = selectCasualtyLocalFireSupportPosition(
        coverer.position,
        casualty.position,
        fireTarget,
        occupied,
        this.rescueSecurityReplans,
      );
      support.rescueSupportPosition = selected;
      support.rescueSupportPlanKey = planKey;
      this.v13V11().rescueCoverTarget = clonePoint(selected);
      if (selected !== null) {
        this.v13Host().log('squad', 'twr:rifle-squad-01', 'Rifle Squad 01', 'plan', 'Rescue security selected a casualty-local threat-facing fire-support lane.', {
          downedId: plan.downedAgentId,
          rescuerId: plan.rescuerId,
          covererId: plan.covererId,
    ...
    if (support.rescueSupportPosition !== null) this.v13V11().rescueCoverTarget = { ...support.rescueSupportPosition };

    const member = this.v13Host().members.find((entry) => entry.id === plan.covererId);
  ...
  private handleRescueSecurityWait(state: TacticalWizardSimulationStateV12, plan: RescuePlanAccess): void {
    ...
    support.rescueSupportPosition = null;
    this.v13V11().rescueCoverTarget = null;
    this.v13Host().log('squad', 'twr:rifle-squad-01', 'Rifle Squad 01', 'plan', 'Rescue security lane stalled; one bounded alternate-position replan was requested.', {
      downedId: plan.downedAgentId,
      rescuerId: plan.rescuerId,
      covererId: plan.covererId,
  ...
  private abortRescue(reason: string): void {
    ...
    const plan = v10.rescuePlan;
    if (plan === null) return;
    const tick = this.v13Host().logicalTick;
    this.v13Host().log('squad', 'twr:rifle-squad-01', 'Rifle Squad 01', 'plan', 'Rescue commitment ended cleanly instead of re-entering an unbounded positioning loop.', {
      downedId: plan.downedAgentId,
      rescuerId: plan.rescuerId,
      covererId: plan.covererId,
      reason,
  ...
  private maintainOperationalRoster(replanOnChange: boolean): void {
    ...
    if (!replanOnChange || before === after || host.members.filter((member) => this.isAlive(member.id)).length === 0) return;

    if (host.members.filter((member) => this.isAlive(member.id)).length < 3 && (host.tactic === 'crossfire' || host.tactic === 'assault')) {
    ...
    host.log('squad', 'twr:rifle-squad-01', 'Rifle Squad 01', 'roles', 'Downed members were removed from active perception / maneuver responsibilities.', {
      livingAgents: host.members.filter((member) => this.isAlive(member.id)).map((member) => member.id),
  ...
  private normalizeOperationalRoleIds(): void {
    const host = this.v13Host();
    const living = host.members.filter((member) => this.isAlive(member.id)).sort((left, right) => left.id.localeCompare(right.id, 'en'));
    ...
    const distinct = (...ids: Array<string | null>): Set<string> => new Set(ids.filter((id): id is string => id !== null));
    if (host.suppressorId === null || !livingIds.has(host.suppressorId)) host.suppressorId = living[0]!.id;
    const usedSuppressor = distinct(host.suppressorId);
    if (host.moverId === null || !livingIds.has(host.moverId) || usedSuppressor.has(host.moverId)) {
      host.moverId = living.find((member) => !usedSuppressor.has(member.id))?.id ?? null;
    }
    const usedPrimary = distinct(host.suppressorId, host.moverId);
    if (host.observerId === null || !livingIds.has(host.observerId) || usedPrimary.has(host.observerId)) {
      host.observerId = living.find((member) => !usedPrimary.has(member.id))?.id ?? null;
    }
  }
  ...
  private sanitizeDownedMembers(): void {
    ...
    for (const member of host.members) {
      if (this.isAlive(member.id)) continue;
      member.targetVisible = false;
      member.firePulse = 0;
  ...
  private maintainCounterfire(state: TacticalWizardSimulationStateV12): void {
    ...
    const rescue = this.v13V10().rescuePlan;
    if (!threat.active || threat.phase !== 'break_contact' || threat.estimatedSector === null || rescue !== null) {
      this.clearCounterfire();
      return;
    }
    ...
    const planKey = `${threat.sourceAgentId ?? 'unknown'}:${gridKey(toCell(target))}:${threat.startedTick ?? state.logicalTick}`;
    if (this.counterfirePlanKey !== planKey || this.counterfireAgentId === null || !candidates.some((agent) => agent.id === this.counterfireAgentId)) {
      const occupied = living.map((agent) => agent.position);
      const ranked = candidates
        .map((agent) => {
          const supportPosition = selectCounterfirePosition(agent.position, target, occupied.filter((point) => distance(point, agent.position) > 0.2));
          return {
            agent,
            supportPosition,
            score: supportPosition === null
              ? -9999
              : (hasLineOfSight(tacticalWizardNavigationGrid, toCell(agent.position), toCell(target)) ? 30 : 0)
                - distance(agent.position, supportPosition) * 2,
          };
        })
      ...
      this.v13Host().log('squad', 'twr:rifle-squad-01', 'Rifle Squad 01', 'plan', 'A non-exposed member established counterfire responsibility against the coarse incoming-fire sector.', {
    ...
    const member = this.counterfireAgentId === null ? null : this.v13Host().members.find((entry) => entry.id === this.counterfireAgentId) ?? null;
    ...
    const positionReady = distance(member.position, this.counterfirePosition) <= COUNTERFIRE_ARRIVAL;
    if (!lineOfFire || !positionReady || member.firePulse > 0) return;
    const tick = this.v13Host().logicalTick;
    ...
    this.v13Host().tryFire(member, this.counterfireTarget, 'counter-ambush suppression toward coarse threat sector');
    if (member.firePulse <= beforePulse) return;
    this.lastCounterfireTick = tick;
    this.v13Host().pushEvent(`T${tick}: ${member.label} returns suppressive fire toward the inferred incoming-fire sector while the exposed element breaks contact.`);
  }
  ...
  private isAlive(agentId: string): boolean {
    const state = super.getState();
    return state.agents.some((agent) => agent.id === agentId && agent.alive);
  }
...
function selectCasualtyLocalFireSupportPosition(
  ...
  for (let y = Math.max(0, casualtyCell.y - RESCUE_SECURITY_RADIUS); y <= Math.min(tacticalWizardTestMap.height - 1, casualtyCell.y + RESCUE_SECURITY_RADIUS); y += 1) {
    for (let x = Math.max(0, casualtyCell.x - RESCUE_SECURITY_RADIUS); x <= Math.min(tacticalWizardTestMap.width - 1, casualtyCell.x + RESCUE_SECURITY_RADIUS); x += 1) {
      ...
      const score = adjacentCover * 18
        - movementCost * 2.4
        - standoffPenalty * 5
        + Math.min(10, distance(point, threat)) * 0.15
        + replanBias;
...
function selectCounterfirePosition(from: GridPoint, threat: GridPoint, occupied: readonly GridPoint[]): GridPoint | null {
  ...
  for (let y = Math.max(0, start.y - COUNTERFIRE_RADIUS); y <= Math.min(tacticalWizardTestMap.height - 1, start.y + COUNTERFIRE_RADIUS); y += 1) {
    for (let x = Math.max(0, start.x - COUNTERFIRE_RADIUS); x <= Math.min(tacticalWizardTestMap.width - 1, start.x + COUNTERFIRE_RADIUS); x += 1) {
      ...
      const movementCost = Math.max(0, path.length - 1);
      const adjacentCover = blockedNeighbourCount(point);
      const currentBonus = distance(point, from) <= 0.9 ? 26 : 0;
      const score = currentBonus + adjacentCover * 12 - movementCost * 2.2;
      candidates.push({ point, score });
    }
  }
  candidates.sort((left, right) => right.score - left.score || left.point.y - right.point.y || left.point.x - right.point.x);
