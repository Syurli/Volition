...
interface MutablePlayerGrenade extends GrenadeVisual {
...
/**
 * V9 adds a Host-owned reactive combat layer around V8.
 *
 * It intentionally does not move weapon traces, grenade physics, smoke geometry
 * or mouse input into Portable Core. Those are Host facts. Volition receives and
 * reacts to the resulting tactical context while the existing V8 squad plan stays
 * intact underneath temporary reactions.
 */
export class TacticalWizardSimulation extends TacticalWizardSimulationV8 {
  ...
  constructor() {
    ...
    base.pushEvent('V9: reactive grenade follow-ups, player weapon controls, health and threat-aware dodge enabled.');
    base.log('system', 'simulation', 'Volition Simulation', 'session', 'V9 reactive combat layer enabled.', {
  ...
  override reset(): TacticalWizardSimulationState {
    ...
    const base = this.unsafe();
    base.pushEvent('V9: reactive combat state reset.');
  ...
  override advance(deltaSeconds: number): TacticalWizardSimulationState {
    ...
    if (this.tacticalEffectUntilTick !== null && this.unsafe().logicalTick >= this.tacticalEffectUntilTick) this.clearTacticalEffect();
    return this.getState();
  }
  ...
  override getState(): TacticalWizardSimulationState {
    ...
    return {
      ...
      grenadeEvents: [...base.grenadeEvents, ...this.playerGrenades.map((grenade) => ({ ...grenade }))],
      playerCombat: {
  ...
  setPlayerAimTarget(point: GridPoint): boolean {
    ...
    this.playerFacing = normalize({ x: dx, y: dy });
    return true;
  }
  ...
  playerFireAt(point: GridPoint): boolean {
    ...
    const direction = this.playerFacing;
    const rayEnd = clampWorldPoint({ x: origin.x + direction.x * PLAYER_WEAPON_RANGE, y: origin.y + direction.y * PLAYER_WEAPON_RANGE });
    ...
    const candidates = state.agents
      ...
      .map((agent) => {
        const sample = pointToSegment(agent.position, origin, rayEnd);
        return { agent, sample };
      })
    ...
    for (const { agent, sample } of candidates) {
      if (hit?.agent.id === agent.id || sample.distance <= PLAYER_NEAR_MISS_RADIUS) this.triggerDodge(agent.id, hit?.agent.id === agent.id ? 'player_hit' : 'player_near_miss');
    }
  ...
  cyclePlayerGrenade(delta: number): GrenadeKind {
    ...
    this.unsafe().log('player', 'player', 'Player', 'plan', `Player selected ${selected} grenade.`, { selectedGrenade: selected });
    return selected;
  }
  ...
  playerThrowGrenadeAt(point: GridPoint): boolean {
    ...
    const delta = { x: requested.x - origin.x, y: requested.y - origin.y };
    const range = Math.hypot(delta.x, delta.y);
    if (range < 0.25) return false;
    const scale = Math.min(1, PLAYER_GRENADE_RANGE / range);
    const target = clampWorldPoint({ x: origin.x + delta.x * scale, y: origin.y + delta.y * scale });
  ...
  /** Workbench/debug hook for scenario authoring and deterministic regression tests. */
  setAgentVitals(agentId: string, values: { readonly health?: number; readonly moveSpeed?: number }): boolean {
    ...
    if (values.moveSpeed !== undefined && Math.abs(values.moveSpeed - current.moveSpeed) > 0.001) return false;
    if (current.health <= 0) this.setReaction(agentId, { kind: 'downed', target: null, source: 'debug', untilTick: Number.MAX_SAFE_INTEGER }, true);
    return true;
  }

  /** Scenario-test hook: applies the same linked follow-up used after an AI grenade detonates. */
  applyGrenadeDoctrineForTest(kind: GrenadeKind, center: GridPoint, ownerId = 'twr:rifle-squad:alpha'): void {
    this.applyAiGrenadeDoctrine({
      id: 900000 + this.unsafe().logicalTick,
      ownerId,
      kind,
  ...
  private installV9Hooks(): void {
    const host = this.unsafe();
    const originalMovementTarget = host.movementTarget.bind(this);
    host.movementTarget = (member: UnsafeMember): GridPoint | null => {
    ...
    host.tryFire = (member: UnsafeMember, target: GridPoint, reason: string): void => {
    ...
    host.tryGrenade = (member: UnsafeMember): boolean => {
      const vitals = this.vitals.get(member.id);
      const reaction = this.activeReaction(member.id, host.logicalTick);
  ...
  private initializeVitals(): void {
    this.vitals = new Map(this.unsafe().members.map((member) => [member.id, {
  ...
  private reassignRolesAfterDowned(downedId: string): void {
    ...
    const alive = host.members.filter((member) => member.id !== downedId && (this.vitals.get(member.id)?.health ?? 0) > 0).map((member) => member.id);
  ...
  private applyPlayerGrenadeDetonation(grenade: MutablePlayerGrenade): void {
    ...
    if (grenade.kind === 'flash') {
      for (const agent of state.agents) {
        ...
        host.log('agent', agent.id, agent.label, 'perception', `${agent.label} was stunned by the player's flash grenade.`, { reaction: 'stunned', durationTicks: duration, range: Number(range.toFixed(2)) });
      }
  ...
  private observeAiGrenadeDetonations(): void {
    ...
    for (const grenade of state.grenadeEvents) {
      if (grenade.ownerId === 'player') continue;
  ...
  private setReaction(agentId: string, reaction: MutableReaction, force = false): void {
    const current = this.activeReaction(agentId, this.unsafe().logicalTick);
  ...
  private cleanupExpiredReactions(): void {
    const tick = this.unsafe().logicalTick;
  ...
  private segmentObscuredBySmoke(from: GridPoint, to: GridPoint): boolean {
    const baseSmoke = super.getState().grenadeEvents.filter((grenade) => grenade.kind === 'smoke' && grenade.totalFrames - grenade.remainingFrames >= grenade.flightFrames);
    const playerSmoke = this.playerGrenades.filter((grenade) => grenade.kind === 'smoke' && grenade.detonated);
    return [...baseSmoke, ...playerSmoke].some((grenade) => this.smokeAffectsLine(grenade.to, grenade.radius, from, to));
  }
  ...
  private isPlayerAimingAt(agentPosition: GridPoint): boolean {
    ...
    const dot = clamp(direction.x * this.playerFacing.x + direction.y * this.playerFacing.y, -1, 1);
  ...
  private memberById(id: string): UnsafeMember | undefined {
    return this.unsafe().members.find((member) => member.id === id);
  }

  private unsafe(): UnsafeHost {
    return this as unknown as UnsafeHost;
  }
}
