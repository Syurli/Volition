...
export interface TacticalWizardSimulationState {
  ...
  /** Compatibility projection for the first Alpha member and existing inspector surfaces. */
...
export const tacticalWizardTestMap = {
  id: 'tactical-wizard-squad-training-ground',
  name: 'Tactical Wizard Squad Training Ground',
  width: 48,
  height: 30,
  blocked: [
    ...rect(8, 3, 4, 7),
...
export class TacticalWizardSimulation {
  ...
  reset(): TacticalWizardSimulationState {
    ...
    this.coverSlots = [];
    this.eventLog.splice(0, this.eventLog.length, 'Simulation reset. Squad patrol initialized.');
    return this.getState();
  }

  emitNoise(intensity = 1): void {
    this.pendingNoiseIntensity = Math.max(this.pendingNoiseIntensity, clamp01(intensity));
    this.pushEvent(`T${this.logicalTick}: player test noise emitted.`);
  }

  setPlayerPosition(point: GridPoint): boolean {
    if (!isWalkable(tacticalWizardNavigationGrid, point)) return false;
    if (this.members.some((member) => samePoint(member.position, point))) return false;
    this.player = point;
    return true;
  }

  /** Direct keyboard movement also creates a low-intensity footstep stimulus. */
  nudgePlayer(dx: number, dy: number): boolean {
    const moved = this.setPlayerPosition({ x: this.player.x + dx, y: this.player.y + dy });
  ...
  step(): TacticalWizardSimulationState {
    ...
    if (this.alertState === 'active') this.ensureTacticalPlan();
    ...
    for (const member of this.members) {
      ...
      const snapshot = member.runtime.tick({
        tick: { logicalTick: this.logicalTick, deltaSeconds: this.logicalTick === 0 ? 0 : this.stepSeconds, seed: 1337 },
    ...
    for (const member of [...this.members].sort((left, right) => left.id.localeCompare(right.id, 'en'))) {
      occupied.delete(gridKey(member.position));
      this.executeMember(member, occupied);
      occupied.add(gridKey(member.position));
    }

    if (!occupiedPositionsAreUnique(this.members.map((member) => member.position))) {
      throw new Error('Simulation invariant violated: squad members overlap.');
    }
  ...
  private updateSquadAlert(visibility: ReadonlyMap<string, boolean>): void {
    const reporter = [...this.members]
      .filter((member) => visibility.get(member.id) === true)
  ...
  private applyRoles(): void {
    for (const member of this.members) {
      member.role = member.id === this.suppressorId
        ? 'suppressor'
        : member.id === this.moverId
          ? 'mover'
          : member.id === this.observerId
            ? 'observer'
            : 'patrol';
    }
  }
  ...
  private ensureTacticalPlan(): void {
    ...
    if (this.coverSlots.length === 0 || this.members.some((member) => member.role !== 'patrol' && member.tacticalTarget === null)) {
      this.refreshCoverPlan();
    }
  }
  ...
  private refreshCoverPlan(): void {
    ...
    this.coverSlots = discoverCoverSlots(tacticalWizardNavigationGrid, threat);
    ...
    for (const id of order) {
      ...
      const slot = selectCoverSlot(tacticalWizardNavigationGrid, this.coverSlots, member.position, threat, reserved, mode);
      member.coverSlot = slot;
      if (slot !== null) reserved.add(slot.id);
      if (member.role === 'suppressor') {
        member.tacticalTarget = hasLineOfSight(tacticalWizardNavigationGrid, member.position, threat)
          ? null
          : slot?.peekPosition ?? null;
      } else if (member.role === 'mover') {
        member.tacticalTarget = slot?.position ?? null;
  ...
  private advanceBoundingIfReady(): void {
    if (this.alertState !== 'active' || this.moverId === null || this.suppressorId === null) return;
    if (this.logicalTick - this.phaseStartedTick < MIN_BOUNDING_PHASE_TICKS) return;
    ...
    this.applyRoles();
    this.refreshCoverPlan();
    const suppressor = this.members.find((member) => member.id === this.suppressorId);
    const nextMover = this.members.find((member) => member.id === this.moverId);
  ...
  private buildStimuli(member: MutableMember, visible: boolean): readonly Stimulus[] {
    ...
    if (visible) {
      stimuli.push({
        id: `visual:player:${member.id}:${this.logicalTick}`,
        sequence: 20,
        logicalTick: this.logicalTick,
        kind: 'visual_actor',
        actorId: 'player',
        visible: true,
        position: toVector3(this.player),
        relation: 'hostile',
      });
      if (!member.wasVisible) this.pushEvent(`T${this.logicalTick}: ${member.label} visual contact confirmed.`);
    } else if (member.wasVisible) {
      stimuli.push({
        id: `visual:player:${member.id}:${this.logicalTick}:lost`,
        sequence: 20,
        logicalTick: this.logicalTick,
        kind: 'visual_actor',
        actorId: 'player',
    ...
    if (this.pendingNoiseIntensity > 0 && distance(member.position, this.player) <= this.hearingRadius) {
      ...
      stimuli.push({
        id: `noise:player:${member.id}:${this.logicalTick}`,
        sequence: 10,
        logicalTick: this.logicalTick,
        kind: 'noise',
        sourceId: 'player',
        perceivedPosition: toVector3(perceived),
        intensity: this.pendingNoiseIntensity,
        actionKind: this.pendingNoiseIntensity >= 0.8 ? 'manual_test_noise' : 'footstep',
      });
    }

    if (this.alertState === 'active' && this.sharedLastKnownPosition !== null && member.id !== this.alertSourceId) {
      stimuli.push({
        id: `squad-report:${member.id}:${this.logicalTick}`,
        sequence: 15,
        logicalTick: this.logicalTick,
        kind: 'squad_report',
        sourceId: this.alertSourceId ?? 'squad',
        subjectId: 'player',
        reportedPosition: toVector3(this.sharedLastKnownPosition),
        confidence: 0.82,
      });
    }
    return stimuli;
  ...
  private executeMember(member: MutableMember, occupied: Set<string>): void {
    if (this.alertState === 'active' && this.sharedLastKnownPosition !== null) {
      if (member.role === 'mover') {
        if (member.coverSlot !== null) this.moveMemberToward(member, member.coverSlot.position, occupied);
      ...
      if (member.role === 'observer') {
        if (member.coverSlot !== null) this.moveMemberToward(member, member.coverSlot.position, occupied);
        this.faceToward(member, this.sharedLastKnownPosition);
        if (member.targetVisible && this.logicalTick % 5 === 0) this.fireAt(member, this.player, 'observer support');
        return;
      }
      if (member.role === 'suppressor') {
        if (member.tacticalTarget !== null) this.moveMemberToward(member, member.tacticalTarget, occupied);
        this.faceToward(member, this.sharedLastKnownPosition);
        if (hasLineOfSight(tacticalWizardNavigationGrid, member.position, this.sharedLastKnownPosition) && this.logicalTick % 3 === 0) {
          this.fireAt(member, this.sharedLastKnownPosition, 'suppressive fire');
        }
        return;
      }
    }

    if (member.selectedIntent === 'engage' && member.targetVisible) {
      this.faceToward(member, this.player);
      member.path = [];
      if (this.logicalTick % 2 === 0) this.fireAt(member, this.player, 'engage');
      return;
    }

    if (member.selectedIntent === 'search') member.searchPulse = 6;
    let target = member.latestSnapshot?.selectedIntent.targetPosition === undefined
      ? null
      : fromVector3(member.latestSnapshot.selectedIntent.targetPosition);
    if (member.selectedIntent === 'patrol') target = this.getPatrolTarget(member);
    if (target === null) {
      member.path = [];
      return;
    }
    this.moveMemberToward(member, target, occupied);
  }

  private moveMemberToward(member: MutableMember, target: GridPoint, occupied: ReadonlySet<string>): void {
    const path = findPath(tacticalWizardNavigationGrid, member.position, target);
    member.path = path;
    if (path.length <= 1) return;
    const next = path[1]!;
    if (occupied.has(gridKey(next)) || samePoint(next, this.player)) return;
    member.facing = { x: next.x - member.position.x, y: next.y - member.position.y };
    member.position = next;
    member.path = path.slice(1);
  }

  private advancePatrolIfReady(): void {
    if (this.alertState !== 'idle') return;
    if (!this.members.every((member) => distance(member.position, this.getPatrolTarget(member)) <= 1)) return;
  ...
  private getPatrolTarget(member: MutableMember): GridPoint {
    ...
    const candidates = [
      { x: base.x + member.patrolOffset.x, y: base.y + member.patrolOffset.y },
      { x: base.x + member.patrolOffset.x, y: base.y },
      { x: base.x, y: base.y + member.patrolOffset.y },
      base,
    ];
  ...
  private canSeePlayer(member: MutableMember): boolean {
    ...
    if (range > this.visionRange) return false;
    if (!hasLineOfSight(tacticalWizardNavigationGrid, member.position, this.player)) return false;
    ...
    return angle <= this.visionFovDegrees / 2;
  }

  private faceToward(member: MutableMember, point: GridPoint): void {
    const dx = Math.sign(point.x - member.position.x);
    const dy = Math.sign(point.y - member.position.y);
    if (dx !== 0 || dy !== 0) member.facing = { x: dx, y: dy };
  }
  ...
  private noisyPerceivedPosition(member: MutableMember): GridPoint {
    ...
    const candidate = { x: this.player.x + offset.x, y: this.player.y + offset.y };
    return isWalkable(tacticalWizardNavigationGrid, candidate) ? candidate : this.player;
  }
...
function rect(x: number, y: number, width: number, height: number): GridPoint[] {
  ...
  for (let yy = y; yy < y + height; yy += 1) for (let xx = x; xx < x + width; xx += 1) points.push({ x: xx, y: yy });
  return points;
}
