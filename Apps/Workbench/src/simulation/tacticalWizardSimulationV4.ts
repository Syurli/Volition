...
export type { SimulationOverlaySettings, TacticalWizardSimulationState } from './tacticalWizardSimulationV3';
...
const ARRIVAL_RADIUS = 0.55;
const ALERT_PROPAGATION_TICKS = 2;
const ALERT_MEMORY_TICKS = 56;
const MIN_BOUNDING_PHASE_TICKS = 4;
const STALLED_REPLAN_FRAMES = 60;
const RUN_LOG_LIMIT = 9000;
const MOTION_LOG_EVERY_FRAMES = 3;
...
export class TacticalWizardSimulation {
  ...
  private tacticReason = 'Initial doctrine: bounded advance.';
  ...
  private readonly eventLog: string[] = ['Simulation ready. 30 Hz motion / 4 Hz decision clocks initialized.'];
  ...
  constructor() {
    this.log('system', 'simulation', 'Volition Simulation', 'session', 'Reference simulation session started with decoupled motion and decision clocks.', {
  ...
  reset(): TacticalWizardSimulationState {
    ...
    this.tacticReason = 'Initial doctrine: bounded advance.';
    ...
    this.eventLog.splice(0, this.eventLog.length, 'Simulation reset. 30 Hz motion / 4 Hz decision clocks initialized.');
    this.runLog = [];
    this.runLogSequence = 0;
    this.log('system', 'simulation', 'Volition Simulation', 'session', 'Simulation reset; new run log started.', {
      map: tacticalWizardTestMap.id,
      motionHz: MOTION_HZ,
      decisionHz: 1 / DECISION_INTERVAL_SECONDS,
      agentMoveSpeed: AGENT_MOVE_SPEED,
  ...
  private advanceDecisionTick(): void {
    ...
    for (const member of this.members) {
      ...
      const values = {
        ...
        ...(member.coverSlot === null ? {} : { coverTarget: toVector3(member.coverSlot.position) }),
      };
  ...
  private advanceMotionFrame(): void {
    for (const member of this.members) {
      ...
      if (member.firePulse === 0) member.fireTarget = null;
    ...
    for (const member of [...this.members].sort((left, right) => left.id.localeCompare(right.id, 'en'))) {
      ...
      if (target !== null) this.moveMemberToward(member, target, occupiedCells, AGENT_MOVE_SPEED * MOTION_STEP_SECONDS);
      occupiedCells.add(gridKey(toNavCell(member.position)));
      if (this.alertState === 'active' && this.sharedLastKnownPosition !== null) this.faceToward(member, this.sharedLastKnownPosition);
    }
  ...
  private movementTarget(member: MutableMember): GridPoint | null {
    if (this.alertState === 'active' && this.sharedLastKnownPosition !== null) return member.tacticalTarget;
    return this.getPatrolTarget(member);
  }
  ...
  private executeDecisionEffects(): void {
    ...
    for (const member of this.members) {
      if (this.tactic === 'sweep') {
        member.searchPulse = Math.max(member.searchPulse, 10);
        if (this.logicalTick % 4 === 0) {
          this.log('agent', member.id, member.label, 'search', `${member.label} is sweeping a committed LKP sector.`, { role: member.role, target: member.tacticalTarget ?? null });
        }
        continue;
      }
      const line = hasWorldLineOfSight(member.position, this.sharedLastKnownPosition);
      const settled = member.tacticalTarget !== null && distance(member.position, member.tacticalTarget) <= ARRIVAL_RADIUS + 0.25;
      if (this.tactic === 'bounding' && member.role === 'suppressor' && line && this.logicalTick % 2 === 0) this.fireAt(member, this.sharedLastKnownPosition, 'suppressive fire');
      else if (this.tactic === 'flank' && member.role === 'suppressor' && line && this.logicalTick % 2 === 0) this.fireAt(member, this.sharedLastKnownPosition, 'fixing fire');
      else if (this.tactic === 'flank' && member.role === 'flanker' && settled && line && this.logicalTick % 2 === 0) this.fireAt(member, this.sharedLastKnownPosition, 'flanking fire');
      else if (this.tactic === 'crossfire' && member.role === 'crossfire' && settled && line && this.logicalTick % 2 === 0) this.fireAt(member, this.sharedLastKnownPosition, 'crossfire');
      else if (this.tactic === 'assault' && member.role === 'assaulter' && line && this.logicalTick % 2 === 0) this.fireAt(member, this.sharedLastKnownPosition, 'assault');
      else if (member.targetVisible && this.logicalTick % 5 === 0) this.fireAt(member, this.player, `${this.tactic} support`);
    }
  }
  ...
  private activateSquad(): void {
    ...
    this.tacticReason = 'Contact confirmed; establish initial bounded pressure.';
    this.applyRoles();
    ...
    this.log('squad', 'twr:rifle-squad-01', 'Rifle Squad 01', 'alert', 'Shared combat alert activated.', { source: source.id, tactic: this.tactic });
  ...
  private transitionTactic(next: SquadTactic, reason: string, rotateRoles: boolean): void {
    ...
    this.log('squad', 'twr:rifle-squad-01', 'Rifle Squad 01', 'tactic', `Tactic ${previous} → ${next}.`, { from: previous, to: next, reason, cycle: this.maneuverCycle });
    this.logRoleAssignments();
  }
  ...
  private planCompletion(): number {
    ...
    if (this.tactic === 'bounding') return settled(this.moverId) ? 1 : 0;
    if (this.tactic === 'flank') return settled(this.moverId) ? 1 : 0;
    if (this.tactic === 'crossfire' || this.tactic === 'assault' || this.tactic === 'sweep') return settled(this.moverId) && settled(this.observerId) ? 1 : 0;
  ...
  private ensureTacticalPlan(): void {
    ...
    if (hardStall) this.pushEvent(`T${this.logicalTick}: tactical element stalled for two seconds; ${this.tactic} plan recalculated.`);
    this.refreshTacticalPlan();
  }
  ...
  private refreshBoundingPlan(): void {
    ...
    const threatCell = toNavCell(threat);
    this.coverSlots = discoverCoverSlots(tacticalWizardNavigationGrid, threatCell);
    this.resetMemberPlans();
    ...
    for (const id of [this.moverId, this.observerId, this.suppressorId].filter((value): value is string => value !== null)) {
      ...
      const slot = selectCoverSlot(tacticalWizardNavigationGrid, this.coverSlots, toNavCell(member.position), threatCell, reserved, mode);
  ...
  private refreshFlankPlan(): void {
    ...
    for (const member of this.members) {
      if (member.id === this.moverId) {
        const point = selectSectorPoint(tacticalWizardNavigationGrid, toNavCell(member.position), toNavCell(threat), toNavCell(origin), reserved, { angleOffsetDegrees: side * 122, angleToleranceDegrees: 34, minRange: 5, maxRange: 9, desiredRange: 6.5, requireLineOfSight: true, minMoveDistance: 4 });
        member.tacticalTarget = point ?? this.fallbackFlank(member, threat, side);
        reserve(reserved, member.tacticalTarget);
      } else if (member.id === this.suppressorId) {
        const slot = selectCoverSlot(tacticalWizardNavigationGrid, this.coverSlots, toNavCell(member.position), toNavCell(threat), new Set<string>(), 'support');
        member.coverSlot = slot;
        ...
        const point = selectSectorPoint(tacticalWizardNavigationGrid, toNavCell(member.position), toNavCell(threat), toNavCell(origin), reserved, { angleOffsetDegrees: -side * 48, angleToleranceDegrees: 42, minRange: 8, maxRange: 13, desiredRange: 10, preferCovered: true, minMoveDistance: 2.5 });
  ...
  private refreshCrossfirePlan(): void {
    ...
    shooters.forEach((id, index) => {
      ...
      const offset = (index === 0 ? side : -side) * 102;
      const point = selectSectorPoint(tacticalWizardNavigationGrid, toNavCell(member.position), toNavCell(threat), toNavCell(origin), reserved, { angleOffsetDegrees: offset, angleToleranceDegrees: 34, minRange: 5, maxRange: 9, desiredRange: 6.8, requireLineOfSight: true, minMoveDistance: 3 });
      member.tacticalTarget = point ?? this.fallbackFlank(member, threat, index === 0 ? side : -side);
      reserve(reserved, member.tacticalTarget);
    });
    ...
    if (support !== undefined) {
      ...
      support.tacticalTarget = point ?? { ...support.position };
      reserve(reserved, support.tacticalTarget);
    }
  }
  ...
  private refreshAssaultPlan(): void {
    ...
    assaulters.forEach((id, index) => {
      const member = this.members.find((entry) => entry.id === id);
      if (member === undefined) return;
      const offset = (index === 0 ? side : -side) * 42;
      const point = selectSectorPoint(tacticalWizardNavigationGrid, toNavCell(member.position), toNavCell(threat), toNavCell(origin), reserved, { angleOffsetDegrees: offset, angleToleranceDegrees: 38, minRange: 2.5, maxRange: 4.75, desiredRange: 3.4, requireLineOfSight: true, minMoveDistance: 1.5 });
      member.tacticalTarget = point ?? { ...member.position };
      reserve(reserved, member.tacticalTarget);
    });
    const support = this.suppressorId === null ? undefined : this.members.find((entry) => entry.id === this.suppressorId);
  ...
  private refreshSweepPlan(): void {
    ...
    sweepers.forEach((id, index) => {
      const member = this.members.find((entry) => entry.id === id);
      if (member === undefined) return;
      const offset = (index === 0 ? side : -side) * 96;
      const point = selectSectorPoint(tacticalWizardNavigationGrid, toNavCell(member.position), toNavCell(lkp), toNavCell(origin), reserved, { angleOffsetDegrees: offset, angleToleranceDegrees: 48, minRange: 3, maxRange: 8, desiredRange: 5.5, minMoveDistance: 3 });
      member.tacticalTarget = point ?? { ...member.position };
      reserve(reserved, member.tacticalTarget);
    });
    const support = this.suppressorId === null ? undefined : this.members.find((entry) => entry.id === this.suppressorId);
  ...
  private refreshRegroupPlan(): void {
    ...
    this.resetMemberPlans();
    const reserved = new Set<string>();
    const offsets = [side * 72, 0, -side * 72];
    this.members.forEach((member, index) => {
      const point = selectSectorPoint(tacticalWizardNavigationGrid, toNavCell(member.position), toNavCell(threat), toNavCell(origin), reserved, { angleOffsetDegrees: offsets[index] ?? 0, angleToleranceDegrees: 48, minRange: 10, maxRange: 16, desiredRange: 12, preferCovered: true, minMoveDistance: 3 });
      if (point !== null) {
        member.tacticalTarget = point;
  ...
  private fallbackFlank(member: MutableMember, threat: GridPoint, side: number): GridPoint {
    const slot = selectCoverSlot(tacticalWizardNavigationGrid, this.coverSlots, toNavCell(member.position), toNavCell(threat), new Set<string>(), 'flank', side);
    member.coverSlot = slot;
  ...
  private advanceBoundingIfReady(): void {
    ...
    this.pushEvent(`T${this.logicalTick}: bounding phase ${this.boundingPhase}; mover completed position and role handoff committed.`);
    this.logRoleAssignments();
    this.logPlan();
  }
  ...
  private moveMemberToward(member: MutableMember, target: GridPoint, occupiedCells: ReadonlySet<string>, maxDistance: number): void {
    ...
    if (Math.abs(dx) > POSITION_EPSILON || Math.abs(dy) > POSITION_EPSILON) {
      ...
      if (this.motionFrame % MOTION_LOG_EVERY_FRAMES === 0) {
        this.log('agent', member.id, member.label, 'move', `${member.label} moved as ${member.role}.`, { from: { ...previous }, to: { ...member.position }, tacticalTarget: { ...target }, role: member.role, tactic: this.tactic, motionFrame: this.motionFrame });
      }
...
function roundWorldPoint(point: GridPoint): GridPoint { return { x: Math.round(point.x * 20) / 20, y: Math.round(point.y * 20) / 20 }; }
