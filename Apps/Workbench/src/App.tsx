...
const TEST_LOADOUT_STORAGE_KEY = 'willform.workbench.tacticalWizardTestLoadout';
...
export function App() {
  ...
  const [config, setConfig] = useState<WillformProjectConfig>(() => structuredClone(tacticalWizardExampleProject.config));
  ...
  const [endpoint, setEndpoint] = useState('wss://localhost:7443/willform');
  ...
  useEffect(() => {
    ...
    window.addEventListener('willform-sim-aim', aim as EventListener);
    ...
    return () => {
      ...
      window.removeEventListener('willform-sim-aim', aim as EventListener);
  ...
  const changeLocale = (next: Locale) => { setLocale(next); window.localStorage.setItem('willform.workbench.locale', next); };
  const connect = () => {
