...
import { SimulationPage, DebugPage } from './pages/RuntimeResponsePages';
import { VisualizationPage, ConnectionPage } from './pages/RuntimePagesBase';
import { RunLogPage } from './pages/RunLogPage';
import type { GridPoint } from './simulation/navigation';
import { TacticalWizardRuntime as TacticalWizardSimulation, type SimulationOverlaySettings, type TacticalWizardSimulationState } from './simulation/tacticalWizardRuntime';
...
export function App() {
  ...
  useEffect(() => { if (!playing) return; const timer = window.setInterval(() => { const beforeTick = simulationRef.current.getState().logicalTick; const next = simulationRef.current.advance(PLAYBACK_FRAME_SECONDS * speed); setSimulation(next); if (next.logicalTick !== beforeTick) collectTraces(next); }, 1000 / PLAYBACK_HZ); return () => window.clearInterval(timer); }, [playing, speed]);

  useEffect(() => {
    const syncSimulation = () => setSimulation(simulationRef.current.getState());
    const keyDown = (event: KeyboardEvent) => { if (page !== 'simulation' || event.target instanceof HTMLInputElement || event.target instanceof HTMLTextAreaElement || event.target instanceof HTMLSelectElement) return; const direction = normalizeDirectionKey(event.key); if (direction !== null) { event.preventDefault(); heldDirections.current = [...heldDirections.current.filter((key) => key !== direction), direction]; } if (event.code === 'Space' && !event.repeat) { event.preventDefault(); simulationRef.current.emitNoise(); syncSimulation(); } };
    ...
    const aim = (event: Event) => { if (page !== 'simulation') return; const point = (event as CustomEvent<GridPoint>).detail; if (point && simulationRef.current.setPlayerAimTarget(point)) syncSimulation(); };
    const fire = (event: Event) => { if (page !== 'simulation') return; const point = (event as CustomEvent<GridPoint>).detail; if (point && simulationRef.current.playerFireAt(point)) syncSimulation(); };
    const throwGrenade = (event: Event) => { if (page !== 'simulation') return; const point = (event as CustomEvent<GridPoint>).detail; if (point && simulationRef.current.playerThrowGrenadeAt(point)) syncSimulation(); };
    const cycleGrenade = (event: Event) => { if (page !== 'simulation') return; const delta = (event as CustomEvent<number>).detail; if (typeof delta === 'number' && delta !== 0) { simulationRef.current.cyclePlayerGrenade(delta); syncSimulation(); } };
    const movementTimer = window.setInterval(() => { if (page !== 'simulation') return; const direction = heldDirections.current.at(-1); if (direction === undefined) return; const [dx, dy] = directionDelta(direction); if (simulationRef.current.nudgePlayer(dx,dy)) syncSimulation(); }, 1000 / PLAYBACK_HZ);
    window.addEventListener('keydown', keyDown); window.addEventListener('keyup', keyUp); window.addEventListener('blur', blur); window.addEventListener('volition-sim-aim', aim as EventListener); window.addEventListener('volition-sim-fire', fire as EventListener); window.addEventListener('volition-sim-grenade', throwGrenade as EventListener); window.addEventListener('volition-sim-cycle-grenade', cycleGrenade as EventListener);
    return () => { window.clearInterval(movementTimer); heldDirections.current = []; window.removeEventListener('keydown', keyDown); window.removeEventListener('keyup', keyUp); window.removeEventListener('blur', blur); window.removeEventListener('volition-sim-aim', aim as EventListener); window.removeEventListener('volition-sim-fire', fire as EventListener); window.removeEventListener('volition-sim-grenade', throwGrenade as EventListener); window.removeEventListener('volition-sim-cycle-grenade', cycleGrenade as EventListener); };
  }, [page]);
  ...
  const connect = () => { const preview = validateLiveEndpoint(endpoint, window.location.protocol); if (!preview.valid) { setConnectionState('error'); setConnectionDetail(preview.message); return; } connection.connect(endpoint, (message) => { setLiveMessages((count) => count + 1); setLiveTelemetry((current) => reduceLiveTelemetry(current, message)); }, (state, detail) => { setConnectionState(state); setConnectionDetail(detail); if (state === 'offline') setLiveTelemetry(EMPTY_LIVE_TELEMETRY); }); };
  ...
  return <div className="editor-shell"><aside className="sidebar"><div className="brand-block"><span className="brand-mark">V</span><div><strong>{t('product')}</strong><small>{t('workbench')}</small></div></div><button className="project-switcher" onClick={() => setPage('projects')}><span><small>{project.kind === 'built-in' ? t('builtInExample') : t('localProject')}</small><strong>{locale === 'zh-CN' && project.nameZh ? project.nameZh : project.name}</strong></span><span>⌄</span></button><nav className="nav-list"><Nav id="overview" current={page} setPage={setPage} icon="◫" label={t('overview')} /><Nav id="design" current={page} setPage={setPage} icon="◇" label={t('design')} /><Nav id="simulation" current={page} setPage={setPage} icon="▶" label={t('simulation')} /><Nav id="debug" current={page} setPage={setPage} icon="◎" label={t('debug')} /><Nav id="log" current={page} setPage={setPage} icon="≡" label={t('log')} /><Nav id="visualization" current={page} setPage={setPage} icon="⌁" label={t('visualization')} /><div className="nav-divider" /><Nav id="connection" current={page} setPage={setPage} icon="↔" label={t('connection')} /></nav><div className="sidebar-footer"><div className="locale-switch"><button className={locale === 'zh-CN' ? 'active' : ''} onClick={() => changeLocale('zh-CN')}>中文</button><button className={locale === 'en-US' ? 'active' : ''} onClick={() => changeLocale('en-US')}>EN</button></div><small>v{__VOLITION_VERSION__} · {__VOLITION_COMMIT__.slice(0,8)}</small></div></aside><main className="editor-main"><header className="editor-topbar"><div><span className="breadcrumb">{t('product')} / {t(pageLabel(page))}</span><h1>{page === 'projects' ? t('projectLibrary') : locale === 'zh-CN' && project.nameZh ? project.nameZh : project.name}</h1></div><div className="top-status"><span className="status-chip offline">Offline Simulation</span><span className={`status-chip ${connectionState}`}>{connectionState}</span></div></header>
    ...
    {page === 'debug' && <DebugPage t={t} locale={locale} simulation={simulation} traces={traceHistory} />}{page === 'log' && <RunLogPage locale={locale} simulation={simulation} />}{page === 'visualization' && <VisualizationPage t={t} traces={traceHistory} />}{page === 'connection' && <ConnectionPage endpoint={endpoint} setEndpoint={setEndpoint} connectionState={connectionState} connectionDetail={connectionDetail} liveMessages={liveMessages} liveTelemetry={liveTelemetry} onConnect={connect} onDisconnect={disconnect} />}
  </main></div>;
}
