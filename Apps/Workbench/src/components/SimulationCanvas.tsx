...
interface Props { readonly state: TacticalWizardSimulationState; readonly overlays: SimulationOverlaySettings; readonly onSetPlayer: (point: GridPoint) => void; }
...
export function SimulationCanvas({ state, overlays, onSetPlayer }: Props) {
  ...
  const handleClick = (event: MouseEvent<SVGSVGElement>) => {
    const rect = event.currentTarget.getBoundingClientRect(); const x = Math.floor((event.clientX - rect.left) / rect.width * tacticalWizardTestMap.width); const y = Math.floor((event.clientY - rect.top) / rect.height * tacticalWizardTestMap.height); onSetPlayer({ x, y });
  };
  return <svg className="simulation-canvas" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Tactical Wizard squad AI simulation test map" onClick={handleClick}>
    ...
    <text className="sim-squad-banner" x={12} y={20}>Squad: {state.squad.alertState} · {state.squad.tactic} · phase {state.squad.phase}</text>
  </svg>;
}
...
function AgentShape({ agent }: { readonly agent: TacticalWizardAgentView }) {
  ...
  return <g className={`sim-enemy agent-${agent.visualKey} role-${agent.role}`} transform={`translate(${position.x} ${position.y})`}><g transform={`rotate(${angle})`}><polygon points="0,-10 8,9 0,6 -8,9" /></g><text className="sim-label" x={11} y={-10}>{agent.label} · {agent.role} · {agent.selectedIntent}</text></g>;
}

function CoverAssignment({ agent }: { readonly agent: TacticalWizardAgentView }) {
  if (agent.coverTarget === null) return null; const target = center(agent.coverTarget); const agentPoint = center(agent.position);
  return <g className={`sim-cover-assignment agent-${agent.visualKey} role-${agent.role}`}><line x1={agentPoint.x} y1={agentPoint.y} x2={target.x} y2={target.y} /><circle cx={target.x} cy={target.y} r={7} /><text x={target.x + 9} y={target.y + 3}>{agent.label}</text></g>;
}

function visionCone(agent: TacticalWizardAgentView, range: number, fovDegrees: number): string {
  const origin = center(agent.position); const facingAngle = Math.atan2(agent.facing.y, agent.facing.x); const half = fovDegrees * Math.PI / 360; const radius = range * CELL;
  const a = { x: origin.x + Math.cos(facingAngle - half) * radius, y: origin.y + Math.sin(facingAngle - half) * radius }; const b = { x: origin.x + Math.cos(facingAngle + half) * radius, y: origin.y + Math.sin(facingAngle + half) * radius };
