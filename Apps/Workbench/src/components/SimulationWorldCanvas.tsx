...
export function SimulationCanvas({ state, overlays, onSetPlayer, locale }: Props) {
  ...
  return <svg className="simulation-canvas tactical-observer" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Tactical Wizard squad AI tactical observer" onClick={handleClick}>
    ...
    {tacticalWizardTestMap.zones.map((zone) => <g className={`sim-zone zone-${zone.kind}`} key={`zone-${zone.id}`} pointerEvents="none">
      <rect x={zone.x * CELL + 2} y={zone.y * CELL + 2} width={zone.width * CELL - 4} height={zone.height * CELL - 4} rx={7} />
      <text x={zone.x * CELL + 8} y={zone.y * CELL + 14}>{locale === 'zh-CN' ? zone.nameZh : zone.name}</text>
    </g>)}
    ...
    {tacticalWizardTestMap.testPoints.map((point) => <TestPoint key={`test-${point.id}`} point={point.position} label={locale === 'zh-CN' ? point.nameZh : point.name} />)}
    ...
    <g className="sim-tactical-hud" transform="translate(12 12)">
      <rect width={362} height={27} rx={6} />
      <text x={9} y={17}>{localizedTactic(state.squad.tactic, locale)} · {locale === 'zh-CN' ? '安全枪线' : 'safe lanes'} {state.safeFireLanes} · {locale === 'zh-CN' ? '展开' : 'spread'} {state.squad.spread.toFixed(1)} · {locale === 'zh-CN' ? '投掷' : 'grenades'} {state.grenadeEvents.length} · PLAN R{state.coordination.planRevision}</text>
    </g>
  </svg>;
}
...
function TestPoint({ point, label }: { readonly point: GridPoint; readonly label: string }) {
  const p = center(point);
  return <g className="sim-test-point" pointerEvents="none"><circle cx={p.x} cy={p.y} r={5} /><line x1={p.x - 8} y1={p.y} x2={p.x + 8} y2={p.y} /><line x1={p.x} y1={p.y - 8} x2={p.x} y2={p.y + 8} /><text x={p.x + 9} y={p.y + 3}>{label}</text></g>;
}
...
function GrenadeEvent({ grenade, locale }: { readonly grenade: GrenadeVisual; readonly locale: Locale }) {
  ...
  const stroke = grenade.kind === 'flash' ? '#ffe279' : grenade.kind === 'smoke' ? '#b6cedc' : '#ff8b62';
  const label = grenade.kind === 'flash' ? (locale === 'zh-CN' ? '震撼弹' : 'FLASH') : grenade.kind === 'smoke' ? (locale === 'zh-CN' ? '烟幕弹' : 'SMOKE') : (locale === 'zh-CN' ? '破片弹' : 'FRAG');
  const age = grenade.totalFrames - grenade.remainingFrames;
  const flight = Math.max(0, Math.min(1, age / Math.max(1, grenade.flightFrames)));
  const projectile = { x: from.x + (to.x - from.x) * flight, y: from.y + (to.y - from.y) * flight };
  const effectActive = age >= grenade.flightFrames;
  const life = Math.max(0, Math.min(1, grenade.remainingFrames / Math.max(1, grenade.totalFrames - grenade.flightFrames)));
  return <g className={`sim-grenade sim-grenade-${grenade.kind}`} pointerEvents="none">
    <line x1={from.x} y1={from.y} x2={to.x} y2={to.y} stroke={stroke} strokeWidth={2} strokeDasharray="5 5" opacity={effectActive ? 0.26 : 0.88} />
    {!effectActive && <><circle cx={projectile.x} cy={projectile.y} r={5.5} fill={stroke} stroke="#0a1017" strokeWidth={2} /><circle cx={projectile.x} cy={projectile.y} r={10} fill="none" stroke={stroke} strokeWidth={1.4} opacity={0.45} /></>}
    {effectActive && <><circle cx={to.x} cy={to.y} r={grenade.radius * CELL} fill={stroke} fillOpacity={grenade.kind === 'smoke' ? 0.12 : 0.055} stroke={stroke} strokeWidth={2.2} strokeDasharray={grenade.kind === 'smoke' ? '2 4' : '8 5'} opacity={Math.max(0.32, life)} /><circle cx={to.x} cy={to.y} r={Math.max(7, grenade.radius * CELL * (1.15 - life * 0.25))} fill="none" stroke={stroke} strokeWidth={1.2} opacity={Math.max(0.18, life * 0.55)} /></>}
    <rect x={to.x + 8} y={to.y - 21} width={locale === 'zh-CN' ? 42 : 38} height={16} rx={4} fill="#091019" fillOpacity={0.9} stroke={stroke} strokeWidth={0.8} />
    <text x={to.x + 13} y={to.y - 10} fill={stroke} fontSize={9} fontWeight={800}>{label}</text>
  </g>;
}
...
function AgentShape({ agent, locale }: { readonly agent: TacticalWizardAgentView; readonly locale: Locale }) {
  ...
  return <g className={`sim-enemy agent-${agent.visualKey} role-${agent.role} cover-${agent.coverState}`} transform={`translate(${position.x} ${position.y})`}>
    ...
    {agent.fireBlockedByFriend && <text className="sim-fire-blocked" x={11} y={special ? 14 : 3}>{locale === 'zh-CN' ? `枪线阻挡 ×${agent.fireBlockedTicks}` : `NO FIRE ×${agent.fireBlockedTicks}`}</text>}
  </g>;
}
