...
const messages = {
  'zh-CN': {
    ...
    simulationDescription: 'Volition 决定高层认知与战术意图；Workbench Simulation Host 负责 A*、LOS、掩体/扇区位置查询、移动与表现。',
    privacyDescription: '目标不可见后，Volition 只消费成员真实观测或小队共享的 Last Known Position；分区搜索不会读取隐藏玩家的实时坐标。',
  },
  'en-US': {
    ...
    simulationDescription: 'Volition owns high-level cognition and tactical intent; the Workbench Simulation Host owns A*, LOS, cover/sector position queries, movement, and presentation.',
    privacyDescription: 'After target loss, Volition consumes only real member observations or squad-shared Last Known Position; sector sweep never reads hidden live player coordinates.',
  },
} as const;
