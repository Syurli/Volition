...
import type { RunLogCategory, RunLogEntry } from '../simulation/tacticalWizardReferenceModel';
import type { TacticalWizardSimulationState } from '../simulation/tacticalWizardRuntime';
...
export function RunLogPage({ locale, simulation }: { readonly locale: Locale; readonly simulation: TacticalWizardSimulationState }) {
  ...
  return <section className="page-stack run-log-page">
    <div className="section-heading"><div><h2>{L('运行日志', 'Run Log')}</h2><p>{L('固定战术层级运行时保留完整诊断缓冲；导出同时记录每名代理的 Plan / Movement / Weapon 执行合同，并把 30Hz 位移合并为决策 Tick 关键帧。后续复盘以 executionAuthority 为最终执行真相。', 'The fixed tactical hierarchy keeps the full diagnostic buffer and exports each agent\'s Plan / Movement / Weapon execution contract. Motion is compressed into decision-tick keyframes; executionAuthority is the canonical execution truth.')}</p></div><button className="primary-button" onClick={exportLog}>⇩ {L('导出压缩日志', 'Export Compact Log')}</button></div>
