...
export function RunLogPage({ locale, simulation }: { readonly locale: Locale; readonly simulation: TacticalWizardSimulationState }) {
  ...
  return <section className="page-stack run-log-page">
    <div className="section-heading"><div><h2>{L('运行日志', 'Run Log')}</h2><p>{L('记录玩家输入、小队战术、角色变化、弹药预算、补给计划，以及每个士兵代理的感知、决策、移动、射击和搜索。导出的 JSON 可以直接发送给我复盘。', 'Records player input, squad tactics, role changes, ammunition budget, logistics plans, and every soldier agent perception, decision, move, fire and search event. Export the JSON for review.')}</p></div><button className="primary-button" onClick={exportLog}>⇩ {L('导出运行日志', 'Export Run Log')}</button></div>
