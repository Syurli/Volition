...
import type { RunLogCategory, RunLogEntry, TacticalWizardSimulationState } from '../simulation/tacticalWizardSimulationV3';
...
export function RunLogPage({ locale, simulation }: { readonly locale: Locale; readonly simulation: TacticalWizardSimulationState }) {
  ...
  const exportLog = () => {
    const payload = {
      ...
      simulation: {
        ...
        agents: simulation.agents.map((agent) => ({ id: agent.id, label: agent.label, position: agent.position, role: agent.role, intent: agent.selectedIntent, beliefConfidence: agent.beliefConfidence, beliefSource: agent.beliefSource, targetVisible: agent.targetVisible, tacticalTarget: agent.tacticalTarget })),
      },
  ...
  return <section className="page-stack run-log-page">
    <div className="section-heading"><div><h2>{L('运行日志', 'Run Log')}</h2><p>{L('记录玩家输入、小队战术、小队代理角色变化，以及每个士兵代理的感知、决策、移动、射击和搜索。导出的 JSON 可以直接发送给我复盘。', 'Records player input, squad tactics, role changes, and every soldier agent perception, decision, move, fire and search event. Export the JSON for review.')}</p></div><button className="primary-button" onClick={exportLog}>⇩ {L('导出运行日志', 'Export Run Log')}</button></div>
...
function localizeSummary(entry: RunLogEntry, locale: Locale): string {
  ...
  if (entry.event === 'roles') return `小队为 ${localizedTactic(String(entry.data.tactic ?? ''), locale)} 重新分配成员职责。`;
  if (entry.event === 'player_move') return '玩家发生位移。';
  if (entry.event === 'player_noise') return '玩家制造了测试噪声。';
  if (entry.event === 'fire') return `${localizedAssetName(entry.actorId, entry.actorLabel, locale)}执行射击。`;
