...
interface Props {
  readonly t: Translate; readonly locale: Locale; readonly config: WillformProjectConfig; readonly setConfig: (config: WillformProjectConfig) => void;
...
function AgentEditor({ locale, agent, config, drag, dropTarget, setDropTarget, onUpdate, onAssignBehavior }: { readonly locale: Locale; readonly agent: AgentDefinition | undefined; readonly config: WillformProjectConfig; readonly drag: DragPayload | null; readonly dropTarget: string | null; readonly setDropTarget: (value: string | null) => void; readonly onUpdate: (update: (agent: AgentDefinition) => AgentDefinition) => void; readonly onAssignBehavior: (behaviorId: string) => void }) {
...
function writeDrag(event: DragEvent, payload: DragPayload) { const text = JSON.stringify(payload); event.dataTransfer.effectAllowed = payload.kind === 'squad-member' ? 'move' : 'copyMove'; event.dataTransfer.setData('application/x-willform-asset', text); event.dataTransfer.setData('text/plain', text); }
function readDrag(event: DragEvent): DragPayload | null { try { const text = event.dataTransfer.getData('application/x-willform-asset') || event.dataTransfer.getData('text/plain'); return text ? JSON.parse(text) as DragPayload : null; } catch { return null; } }
