...
export function DesignPage({ t, locale, config, setConfig, project, onSave, onReset }: Props) {
  ...
  return <section className="page-stack design-page-v2">
    ...
    {mode === 'assets' ? <div className={`authoring-layout-v2 ${drag ? 'drag-session' : ''}`}>
      <aside className="asset-browser-v2"><div className="asset-browser-head"><strong>{L('项目模块', 'Project Modules')}</strong><small>{name(config.projectId, config.displayName)}</small></div>
...
function SquadEditor({ locale, squad, config, drag, dropTarget, setDropTarget, startDrag, endDrag, onUpdate, onAddAgent, onAssignBehavior, onReorder }: { readonly locale: Locale; readonly squad: SquadDefinition | undefined; readonly config: VolitionProjectConfig; readonly drag: DragPayload | null; readonly dropTarget: string | null; readonly setDropTarget: (value: string | null) => void; readonly startDrag: (event: DragEvent, payload: DragPayload) => void; readonly endDrag: () => void; readonly onUpdate: (update: (squad: SquadDefinition) => SquadDefinition) => void; readonly onAddAgent: (agentId: string) => void; readonly onAssignBehavior: (behaviorId: string) => void; readonly onReorder: (dragged: string, target: string) => void }) {
  ...
  return <div className="typed-editor-v2"><EditorTitle type={L('小队代理', 'SQUAD')} title={name(squad.id, squad.displayName)} id={squad.id} canonical={squad.displayName} onCanonicalChange={(value) => onUpdate((current) => ({ ...current, displayName: value }))} locale={locale} />
    <section className="module-zone-v2"><header><div><strong>{L('成员代理与队形顺序', 'Member proxies & formation order')}</strong><small>{L('拖入 AI 代理新增成员；抓住 ⋮⋮ 拖到另一成员上重新排序。', 'Drop AI Agents to add members; grab ⋮⋮ and drop onto another member to reorder.')}</small></div><span className="module-count">{squad.members.length}</span></header>
      <div className={`member-lane-v2 drop-zone-v2 ${canDropMember ? 'can-drop' : ''} ${dropTarget === 'squad-members' ? 'drop-active' : ''}`} onDragOver={(event) => { if (canDropMember) { event.preventDefault(); event.dataTransfer.dropEffect = drag?.kind === 'agent' ? 'copy' : 'move'; } }} onDragEnter={() => canDropMember && setDropTarget('squad-members')} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDropTarget(null); }} onDrop={(event) => dropMember(event)}>
        {squad.members.map((member, index) => { const agent = config.agents.find((entry) => entry.id === member.agentId); const memberTarget = `member:${member.id}`; return <div key={member.id} className={`module-card-v2 agent-module-v2 ${dropTarget === memberTarget ? 'reorder-target' : ''}`} draggable onDragStart={(event) => startDrag(event, { kind: 'squad-member', id: member.id, squadId: squad.id })} onDragEnd={endDrag} onDragOver={(event) => { if (drag?.kind === 'squad-member') { event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; setDropTarget(memberTarget); } }} onDrop={(event) => { event.stopPropagation(); dropMember(event, member.id); }}><span className="card-grip">⋮⋮</span><small>{L('槽位', 'SLOT')} {index + 1}</small><strong>{name(member.agentId, member.displayName ?? agent?.displayName ?? member.agentId)}</strong><code>{member.agentId}</code><em>{member.preferredRole ?? 'flex'}</em></div>; })}
        <div className={`drop-placeholder-v2 ${canDropMember ? 'ready' : ''}`}><b>＋</b><span>{drag?.kind === 'agent' ? L('松开以加入小队', 'Release to add Agent') : drag?.kind === 'squad-member' ? L('拖到成员卡上调整顺序', 'Drop on a member card to reorder') : L('从左侧拖入 AI 代理', 'Drag AI Agent here')}</span></div>
      </div>
    </section>
    <section className="module-zone-v2"><header><div><strong>{L('小队战术模块', 'Squad tactic modules')}</strong><small>{L('这里只接受作用域为 Squad 的行为模块。', 'Only Squad-scoped behavior modules are accepted here.')}</small></div><span className="module-count">{squad.behaviorIds.length}</span></header><div className={`binding-lane-v2 drop-zone-v2 ${canDropBehavior ? 'can-drop' : drag?.kind === 'behavior' ? 'cannot-drop' : ''} ${dropTarget === 'squad-behaviors' ? 'drop-active' : ''}`} onDragOver={(event) => { if (canDropBehavior) { event.preventDefault(); event.dataTransfer.dropEffect = 'copy'; } }} onDragEnter={() => canDropBehavior && setDropTarget('squad-behaviors')} onDragLeave={() => setDropTarget(null)} onDrop={dropBehavior}>{squad.behaviorIds.map((id) => <div key={id} className="binding-chip-v2"><span><strong>{name(id, behaviorMap.get(id)?.displayName ?? id)}</strong><code>{behaviorMap.get(id)?.hostBehaviorRef ?? id}</code></span><button onClick={() => onUpdate((current) => ({ ...current, behaviorIds: current.behaviorIds.filter((entry) => entry !== id) }))}>×</button></div>)}<div className={`drop-placeholder-v2 ${canDropBehavior ? 'ready' : ''}`}><b>＋</b><span>{canDropBehavior ? L('松开以绑定小队战术', 'Release to bind squad tactic') : L('拖入小队战术', 'Drop squad tactic')}</span></div></div></section>
  </div>;
}
...
function AgentEditor({ locale, agent, config, drag, dropTarget, setDropTarget, onUpdate, onAssignBehavior }: { readonly locale: Locale; readonly agent: AgentDefinition | undefined; readonly config: VolitionProjectConfig; readonly drag: DragPayload | null; readonly dropTarget: string | null; readonly setDropTarget: (value: string | null) => void; readonly onUpdate: (update: (agent: AgentDefinition) => AgentDefinition) => void; readonly onAssignBehavior: (behaviorId: string) => void }) {
  ...
  return <div className="typed-editor-v2"><EditorTitle type={L('AI 代理', 'AI AGENT')} title={name(agent.id, agent.displayName)} id={agent.id} canonical={agent.displayName} onCanonicalChange={(value) => onUpdate((current) => ({ ...current, displayName: value }))} locale={locale} />
    <div className="editor-property-grid-v2"><section className="property-panel-v2"><h4>{L('认知 / 决策', 'Brain / Decision')}</h4><label>{L('认知主管', 'Supervisor')}<select value={agent.supervisorId ?? ''} onChange={(event) => onUpdate((current) => ({ ...current, supervisorId: event.target.value || undefined }))}><option value="">—</option>{(config.supervisors ?? []).map((entry) => <option key={entry.id} value={entry.id}>{name(entry.id, entry.displayName)}</option>)}</select></label><code>{agent.decisionPolicy.id}</code>{['engageScore','searchScore','investigateScore','patrolScore'].map((key) => <Num key={key} label={key} value={Number(agent.decisionPolicy.config?.[key] ?? 0)} step={1} onChange={(value) => setScore(key, value)} />)}</section>
      <section className="property-panel-v2"><h4>{L('记忆与能力', 'Memory & Capabilities')}</h4><Num label={L('每秒衰减', 'Decay / second')} value={agent.memory.decayPerSecond} step={0.01} onChange={(value) => setMemory('decayPerSecond', value)} /><Num label={L('遗忘阈值', 'Forget below')} value={agent.memory.forgetBelowConfidence} step={0.01} onChange={(value) => setMemory('forgetBelowConfidence', value)} /><div className="tag-row">{agent.capabilities.map((value) => <span key={value}>{value}</span>)}</div></section>
    </div>
    <section className="module-zone-v2"><header><div><strong>{L('单兵行为绑定', 'Agent behavior bindings')}</strong><small>{L('只接受 Agent 作用域行为。Intent 仍通过 Host-neutral Behavior Ref 执行。', 'Only Agent-scoped behaviors are accepted. Intent still executes through host-neutral Behavior Refs.')}</small></div><span className="module-count">{agent.behaviorIds?.length ?? 0}</span></header><div className={`binding-lane-v2 drop-zone-v2 ${canDrop ? 'can-drop' : drag?.kind === 'behavior' ? 'cannot-drop' : ''} ${dropTarget === 'agent-behaviors' ? 'drop-active' : ''}`} onDragOver={(event) => { if (canDrop) { event.preventDefault(); event.dataTransfer.dropEffect = 'copy'; } }} onDragEnter={() => canDrop && setDropTarget('agent-behaviors')} onDragLeave={() => setDropTarget(null)} onDrop={(event) => { event.preventDefault(); const payload = readDrag(event); if (payload?.kind === 'behavior' && payload.scope === 'agent') onAssignBehavior(payload.id); setDropTarget(null); }}>{(agent.behaviorIds ?? []).map((id) => <div key={id} className="binding-chip-v2"><span><strong>{name(id, behaviorMap.get(id)?.displayName ?? id)}</strong><code>{behaviorMap.get(id)?.hostBehaviorRef ?? id}</code></span><button onClick={() => onUpdate((current) => ({ ...current, behaviorIds: (current.behaviorIds ?? []).filter((entry) => entry !== id) }))}>×</button></div>)}<div className={`drop-placeholder-v2 ${canDrop ? 'ready' : ''}`}><b>＋</b><span>{canDrop ? L('松开以绑定单兵行为', 'Release to bind behavior') : L('拖入单兵行为', 'Drop agent behavior')}</span></div></div></section>
  </div>;
}
...
function BehaviorEditor({ locale, behavior, onUpdate }: { readonly locale: Locale; readonly behavior: BehaviorDefinition | undefined; readonly onUpdate: (update: (behavior: BehaviorDefinition) => BehaviorDefinition) => void }) {
  ...
  return <div className="typed-editor-v2"><EditorTitle type={behavior.scope === 'squad' ? L('小队战术', 'SQUAD TACTIC') : L('单兵行为', 'AGENT BEHAVIOR')} title={title} id={behavior.id} canonical={behavior.displayName} onCanonicalChange={(value) => onUpdate((current) => ({ ...current, displayName: value }))} locale={locale} /><div className="editor-property-grid-v2"><section className="property-panel-v2"><label>{L('作用域', 'Scope')}<select value={behavior.scope} onChange={(event) => onUpdate((current) => ({ ...current, scope: event.target.value as BehaviorDefinition['scope'] }))}><option value="agent">Agent</option><option value="squad">Squad</option></select></label><label>Intent ID<input value={behavior.intentId ?? ''} onChange={(event) => onUpdate((current) => ({ ...current, intentId: event.target.value || undefined }))} /></label><label>Host Behavior Ref<input value={behavior.hostBehaviorRef} onChange={(event) => onUpdate((current) => ({ ...current, hostBehaviorRef: event.target.value }))} /></label></section><section className="property-panel-v2"><label>{L('所需能力（逗号分隔）', 'Required capabilities')}<input value={behavior.requiredCapabilities.join(', ')} onChange={(event) => onUpdate((current) => ({ ...current, requiredCapabilities: splitList(event.target.value) }))} /></label><label>{L('说明', 'Description')}<textarea value={behavior.description ?? ''} onChange={(event) => onUpdate((current) => ({ ...current, description: event.target.value }))} /></label></section></div><section className="module-zone-v2 contract-zone-v2"><strong>{L('行为契约边界', 'Action Contract Boundary')}</strong><p>{L('这里只定义可移植行为契约和 Host alias；真正的移动、动画、射击与技能实现仍位于 Bridge / 游戏 Host。', 'This defines portable behavior contracts and Host aliases only. Concrete movement, animation, firing and abilities remain in the Bridge / game Host.')}</p></section></div>;
}
...
function ReasonerEditor({ locale, reasoner, onUpdate }: { readonly locale: Locale; readonly reasoner: ReasonerDefinition | undefined; readonly onUpdate: (update: (reasoner: ReasonerDefinition) => ReasonerDefinition) => void }) {
  ...
  return <div className="typed-editor-v2"><EditorTitle type={L('推理器', 'REASONER')} title={title} id={reasoner.id} canonical={reasoner.displayName} onCanonicalChange={(value) => onUpdate((current) => ({ ...current, displayName: value }))} locale={locale} /><label className="wide-field">{L('算法类型', 'Reasoner kind')}<select value={reasoner.kind} onChange={(event) => onUpdate((current) => ({ ...current, kind: event.target.value as ReasonerDefinition['kind'] }))}>{['utility','statechart','htn','behavior-tree','goap','custom'].map((kind) => <option key={kind}>{kind}</option>)}</select></label><section className="module-zone-v2 contract-zone-v2"><strong>{L('专用编辑视图', 'Specialized editor')}</strong><p>{L('Utility、HTN、行为树和 GOAP 将保留各自专用编辑器，不强行转换成一种万能节点。', 'Utility, HTN, Behavior Tree and GOAP keep specialized editors rather than being flattened into one universal graph.')}</p></section></div>;
}

function EditorTitle({ type, title, id, canonical, onCanonicalChange, locale }: { readonly type: string; readonly title: string; readonly id: string; readonly canonical: string; readonly onCanonicalChange: (value: string) => void; readonly locale: Locale }) { return <div className="typed-editor-title-v2"><div><span className="asset-type-v2">{type}</span><h2>{title}</h2><code>{id}</code></div><label>{locale === 'zh-CN' ? '内部 / 英文显示名' : 'Display name'}<input value={canonical} onChange={(event) => onCanonicalChange(event.target.value)} /></label></div>; }
